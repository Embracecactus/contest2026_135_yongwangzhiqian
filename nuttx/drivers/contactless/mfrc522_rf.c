/****************************************************************************
 * drivers/contactless/mfrc522_rf.c
 *
...
#define CONFIG_MFRC522_SPI_FREQ CONFIG_CL_MFRC522_RF_SPI_FREQ
 ...
 * Name: mfrc522_set_rf
 *
 * The owner explicitly enables the field for a bounded operation. Register
 * readback detects a rejected transition; it is not an RF power measurement.
 ****************************************************************************/

static int mfrc522_set_rf(FAR struct mfrc522_dev_s *dev, unsigned long enabled)
{
  uint8_t expected;
  uint8_t actual;
  ...
  expected = enabled ? MFRC522_TX1_RF_EN | MFRC522_TX2_RF_EN : 0;
  if (enabled)
    {
      mfrc522_enableantenna(dev);
    }
  ...
  actual = mfrc522_readu8(dev, MFRC522_TX_CTRL_REG);
  return (actual & (MFRC522_TX1_RF_EN | MFRC522_TX2_RF_EN)) == expected ?
         OK : -EIO;
}
...
int mfrc522_register(FAR const char *devpath, FAR struct spi_dev_s *spi)
{
  ...
  /* Publish an idle reader with its field disabled. Do not expose a
   * device that failed to release RF after initialization.
   */

  ret = mfrc522_set_rf(dev, 0);
  if (ret < 0)
    {
      kmm_free(dev);
      return ret;
