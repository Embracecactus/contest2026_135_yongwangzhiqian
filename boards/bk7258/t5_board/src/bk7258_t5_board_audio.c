/****************************************************************************
 * boards/bk7258/t5_board/src/bk7258_t5_board_audio.c
 *
...
static int bk7258_t5_audio_initialize(
  FAR const struct bk7258_aud_config_s *config)
{
   ...
   * required enable-then-set ordering. */
...
static int bk7258_t5_audio_set(
  FAR const struct bk7258_aud_config_s *config, bool enable)
{
  ...
  ret = bk7258_t5_audio_initialize(config);
  if (ret < 0)
    {
      return ret;
    }
...
static bool bk7258_t5_audio_is_enabled(
  FAR const struct bk7258_aud_config_s *config)
{
  bool high;
...
const struct bk7258_aud_board_s g_bk7258_board_audio =
{
  ...
  .is_enabled = bk7258_t5_audio_is_enabled,
};
