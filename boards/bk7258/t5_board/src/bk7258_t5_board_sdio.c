...
static int bk7258_t5_board_tf_mount_worker(int argc, char *argv[])
{
   ...
   * enumeration.  NuttX FAT accepts both a raw FAT volume and a DOS MBR on
   * the parent block device, so no product-private partition state is needed.
   */
      ...
      if (stat(T5_BOARD_TF_BLOCKDEV, &statbuf) == 0)
        {
          ...
          if (ret == OK || ret == -EBUSY)
            {
              syslog(LOG_INFO, "T5-Board TF: mounted %s at %s\n",
                     T5_BOARD_TF_BLOCKDEV, T5_BOARD_TF_MOUNTPOINT);
              return OK;
            }
