...
| Display connector | not fitted | RGB LCD fitted | two fitted GC9D01 160×160 panels; optional CN5 single-screen module disconnected |
| DVP camera connector | not fitted | fitted | GC2145 connected; current product captures JPEG through the single V4L2 owner |
| Motor | not fitted | not fitted | CN10 motor fitted, P9 active high; bounded product vibration |
...
The maintained AP profile binds SC7A20H on I2C0 P20/P21 to the acceleration
service. MFRC522 driver/bus adaptation on UART1 P0/P1 is present, but NFC is
not used in the current contest product interaction; a probe or device node
does not establish a working tap-to-claim flow.
...
The unpopulated X2/C16/C17 network does not reserve P8/P9: P8 is KEY3 and P9
belongs to the fitted motor. The optional CN5 single-screen module is
disconnected; the two GC9D01 eye panels are separate bindings and are active.
The AIDK CP SDK overlay also disables the
vendor MP_A external-32-kHz override and retains the calibrated internal ROSC
as the low-power clock source.

The current GC2145 product route is no longer Phase-0-only. P49 supplies the
camera LDOs, P27 carries MCLK and P28 is reset; I2C1 map mode 1 uses P42/P43.
The chip driver and board binding expose `/dev/video0`; the product vision
service owns capture, and Agent requests reuse that owner. JPEG capture and
recognition have real-board evidence; color accuracy is not guaranteed.

Current applications are T5-Board Dolphin and AIToyBoard Shaniu. Shaniu uses
K1/K3 for volume, K2 for power policy and the wake word for conversation.
K2's low-power current acceptance remains separate from button detection.
The source declaration of a P40 user GPIO must not be read as a claim that
the board's red/green charging LEDs can be switched off in software.

Three-board build commands and current publication limitations are in the
root README. Normal SDK pairs are `cp + ap` for Core/Board and
`cp-aidk + ap-aidk` for AIToyBoard. Firmware 635 Skill/voice/display and
firmware 634 App OTA have separate evidence boundaries in the Master Plan.
