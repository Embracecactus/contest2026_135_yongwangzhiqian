...
- Active branch: \`refactor/bk7258-scripts-convergence\`.
- Upstream base:
...
- Commit series through P3:
  ...
  P4 is the structural-gate/current-evidence checkpoint containing this file.
...
- Standard unsigned aliases were produced and hash-bound:
...
- Full BK7258 host suite: **167 passed**. Signing, package creation,
  private-key reads, network and hardware remained \`NOT_RUN\`.

## Remaining work

- Push the completed four-commit branch over SSH. The owner will open the PR
  to \`dev-ai-contest-2026\` from the GitHub web interface.
- Production signing, \`firmware.bkpack\` creation, Flash/COM/J-Link and hardware
  validation still require separate authorization.
...
After the SSH push, the owner opens the web PR to \`dev-ai-contest-2026\`.
Do not start the next driver task on this branch.
...
- Do not invoke \`deliver\`, read private keys, sign, create a production
  package, Flash, reset, or access COM/J-Link hardware without new authority.
- Do not claim a signed-package or hardware PASS from the keyless build result.
