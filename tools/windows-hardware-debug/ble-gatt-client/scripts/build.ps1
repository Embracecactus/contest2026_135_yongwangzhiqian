...
<#
.SYNOPSIS
Builds the Windows C++/WinRT BLE GATT client command-line tool.
...
$source = Join-Path $toolRoot 'src\ble_gatt_client.cpp'
$outputDirectory = Join-Path $toolRoot 'out'
$outputExe = Join-Path $outputDirectory 'ble_gatt_client.exe'
$outputObject = Join-Path $outputDirectory 'ble_gatt_client.obj'
$outputPdb = Join-Path $outputDirectory 'ble_gatt_client.pdb'
...
Write-Output ("BLEGATTC BUILD PASS exe='{0}' sha256={1}" -f $outputExe, $hash.Hash.ToLowerInvariant())
