#!/usr/bin/env python3
"""Materialize canonical BK7258 CP/AP configs from a resolved build plan.

The composition framework owns product resolution.  This module is only the
temporary shell adapter: it renders the already-resolved CP/AP IR into a
caller-owned root.  It never reads ``board/bk7258/configs``.  The historical
profile names remain in the freeze/migration ledgers as shadow evidence only.

The historical :func:`materialize` entry point remains available for older
host checks; new callers should use :func:`materialize_plan`.
"""
...
# shell.  They remain data-only metadata; the suite symbols themselves come
# from bk7258_validation_suite_catalog.json through the framework resolver.
...
def seed_record(seed_root: Path, seed_profile: str, overlay: str,
    ...
    result = {
        ...
        "source": framework.CANONICAL_CONFIG_SOURCE,
...
def _validate_plan(plan: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for role in ("cp", "ap"):
        ...
        if (adapter.get("mode") != "shadow-comparator" or
                row["source"] != framework.CANONICAL_CONFIG_SOURCE or
                row["compat"] != framework.CANONICAL_CONFIG_COMPAT):
            raise ValueError(
                "legacy profile materialization is retired; use canonical "
                "product/fragment resolution (legacy names are shadow-only)")
...
def materialize(seed_root: Path, output_root: Path, make_defs: Path) -> Path:
    ...
    raise ValueError(
        ...
        "plan/product fragments (legacy names are shadow-only)")
