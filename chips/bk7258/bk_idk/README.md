...
sdk-profiles/<manifest-version>/
  ...
  ap.config
  ap-aidk.config  # normal AIDK AP profile; selected by openvela_ap/profile.conf
...
the low-power source. AIToyBoard's normal product pair selects `cp-aidk` and
`ap-aidk`, while T5AI-Core and T5-Board select `cp` and `ap`.
Use the selected board's role profiles rather than inferring a profile from
its CPU role alone. `ap-sdio4` is an explicit hardware-capability variant.
