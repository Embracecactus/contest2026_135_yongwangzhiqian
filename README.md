...
- **AIDK AI Toy（傻妞）**：`v18.6.401+638` 签名全镜像（operator
  ...
  ASR 请求瞬时 `ret=-5` 后在同一会话内恢复成功。包身份、构建输入与逐层证据见
...
| 内容 | 已有证据 | 不扩大的结论 |
|---|---|---|
| 638 实机链路 | `18.6.401+638` 签名全镜像（同板恢复）：唤醒→“我在”→ASR→LLM→TTS→播放→免唤醒追问→回待机；KWS 与音量键正常 | 一次 ASR `ret=-5` 瞬时失败后同会话恢复；未测 App OTA；无本机传输日志 |
