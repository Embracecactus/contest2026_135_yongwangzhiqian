...
def compile_harness(repo: Path, output: Path) -> None:
    ...
    boot = repo / "board/bk7258_t5ai/bootloader"
    chip = repo / "board/bk7258_t5ai/chip"
    harness = repo / "board/bk7258_t5ai/scripts/host/bk7258_boot_ota_select_harness.c"
    subprocess.run(
        [
            compiler,
            "-std=c11",
...
def source_contract(repo: Path) -> None:
    paths = {
        "core": repo / "board/bk7258_t5ai/bootloader/boot_ota_select_core.c",
        "header": repo / "board/bk7258_t5ai/bootloader/boot_ota_select_core.h",
...
def verify_boot_sha256(repo: Path, root: Path) -> int:
    ...
    source = repo / "board/bk7258_t5ai/bootloader/boot_sha256.c"
