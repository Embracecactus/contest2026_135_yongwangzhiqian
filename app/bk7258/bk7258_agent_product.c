...
static bool product_keys_step(uint64_t now)
{
  ...
  if (power)
  ...
  else if (steps)
    {
      ...
      if (!ret) ret = bkvoice_volume_store_set(volume);
      g_product_error = ret;
      syslog(ret ? LOG_WARNING : LOG_INFO,
...
static int product_apply_persona(int requested)
{
  ...
  if (!ret) ret = bk7258_preferences_get(&preferences);
  if (!ret && requested >= 0 &&
  ...
  if (!ret) {
    ...
    if (!file) ret = -errno;
      ...
      if (fprintf(file, "# 傻妞\n\n"
                  "你是运行在 openvela 设备上的 AI 伴侣，不是真人。\n"
                  "当前聊天风格：%s\n"
                  "风格只影响表达，不改变官方规则和真实能力边界。\n"
                  "硬件动作必须在本轮真实调用工具且成功后才能说已完成。"
                  "用户要求再来一次、再次振动时必须重新调用工具；"
                  "不得只模仿声音或沿用历史成功。\n",
  ...
  switch (command)
    {
...
#ifdef CONFIG_BK7258_PREFERENCES
        if (!ret) ret = bkagent_memory_commit(atomic_load(&g_active_persona), NULL);
#endif
        break;
#ifdef CONFIG_BK7258_PREFERENCES
      ...
      case BKCONTROL_PERSONA:
        if (value > BK7258_PERSONA_TSUNDERE_LITE) return -EINVAL;
        if (!voice_channel_is_idle()) return -EBUSY;
...
{
  return strdup("[{\"name\":\"device_status\",\"description\":\"Read this device's "
    "speaker volume, mood and cached battery voltage/charging state. Battery "
    "percentage is unavailable: never estimate it from voltage.\",\"input_schema\":"
    TOOL_NO_PARAMS "}"
#ifdef CONFIG_BK7258_MOTION_SERVICE
    ",{\"name\":\"device_motion\",\"description\":\"Read fresh three-axis "
    "acceleration including gravity from this board. It has an accelerometer, "
    "not a gyroscope; cannot measure angular velocity.\",\"input_schema\":"
    TOOL_NO_PARAMS "}"
#endif
    ",{\"name\":\"device_control\",\"description\":\"Only on the user's request, "
    "set speaker volume (value 0-100 percent), mood/chat style (mood), or "
    "vibrate briefly (value 1-100 milliseconds). Every repeat request needs "
    "a fresh tool call; never claim another action based on an earlier success. "
#ifdef CONFIG_BK7258_DISPLAY_SERVICE
    "Use action eyes with expression to show an emotion when requested or "
    "helpful in conversation; local display handles blinking, never call per frame. "
#endif
    "Report errors honestly.\","
    "\"input_schema\":{\"type\":\"object\",\"properties\":{"
    "\"action\":{\"type\":\"string\",\"enum\":[\"volume\",\"mood\",\"vibrate\""
#ifdef CONFIG_BK7258_DISPLAY_SERVICE
    ",\"eyes\""
#endif
    "]},"
#ifdef CONFIG_BK7258_DISPLAY_SERVICE
    "\"expression\":{\"type\":\"string\",\"enum\":[\"neutral\",\"happy\",\"shy\","
    "\"sad\",\"surprised\",\"thinking\",\"listening\",\"speaking\",\"sleepy\"]},"
#endif
    "\"value\":{\"type\":\"integer\",\"minimum\":0,\"maximum\":200},"
    "\"mood\":{\"type\":\"string\",\"enum\":[\"gentle\",\"playful\",\"quiet\","
    "\"serious\",\"tsundere_lite\"]}},\"required\":[\"action\"]}}"
    ",{\"name\":\"read_file\",\"description\":\"Read a UTF-8 text file under "
    "/data/agent. Use it to open a skill document under /data/agent/skills "
    "before following that skill's steps.\",\"input_schema\":{\"type\":"
  ...
  char *output, size_t capacity, int (*check)(void *), void *context)
{
  ...
  int ret;
  int written = 0;
  if (!name || (strcmp(name, "device_status") && strcmp(name, "device_motion") &&
  ...
  if (!strcmp(name, "device_status")) {
    ...
    const char *mood = bk7258_preferences_persona_name(atomic_load(&g_active_persona));
    unsigned int volume;
    ...
    if (!bk7258_health_service_snapshot(&health)) {
      ...
      if (health.flags & BK7258_HEALTH_SNAPSHOT_BATTERY_STATE_VALID) {
        switch (health.battery_state) {
          ...
          case BATTERY_IDLE: state = "idle"; break;
          case BATTERY_FAULT: state = "fault"; break;
        }
      }
    }
#endif
    ...
    written = snprintf(output, capacity, "{\"battery_mv\":%s,"
      "\"battery_state\":\"%s\",\"battery_percent\":null,"
      "\"battery_source\":\"periodic_cache\",\"volume_percent\":%s,\"mood\":\"%s\"}",
    ...
    if (!ret && sample.sensor_status < 0) ret = sample.sensor_status;
    if (ret) goto out;
    written = snprintf(output, capacity, "{\"sensor\":\"accelerometer\","
      "\"gyroscope_available\":false,\"unit\":\"mm/s2\",\"timestamp_us\":%llu,"
      "\"x\":%ld,\"y\":%ld,\"z\":%ld}", (unsigned long long)sample.timestamp_us,
    ...
    if (!strcmp(action->valuestring, "mood") && cJSON_IsString(mood)) {
      ...
      static const char *const expressions[] = {"neutral", "happy", "shy",
        "sad", "surprised", "thinking", "listening", "speaking", "sleepy"};
    ...
    if (!strcmp(action->valuestring, "volume"))
      written = snprintf(output, capacity,
        "{\"ok\":true,\"volume_percent\":%u}", observed);
    else
      written = snprintf(output, capacity, "{\"ok\":true,\"action\":\"%s\"}",
                         action->valuestring);
  } else ret = -ENOTSUP;
  if (!ret && (written < 0 || (size_t)written >= capacity)) ret = -ENOSPC;
out:
  cJSON_Delete(args);
  if (ret) snprintf(output, capacity,
    "Error: device operation failed (%d); do not claim success or invent a reading.", ret);
...
static int product_response_mode(enum bkcontrol_command_e command, uint32_t offset,
  const uint8_t *record, size_t size, struct bkcontrol_status_s *status)
{
#ifdef CONFIG_BK7258_PREFERENCES
  bool enabled = false;
  ...
  if (command != BKCONTROL_CONFIG_BEGIN && command != BKCONTROL_CONFIG_APPLY)
    return -EINVAL;
  if (!g_cloud_loaded) return -EAGAIN;
  if (!voice_channel_is_idle() || bkprov_network_busy()) return -EBUSY;
  ...
  if (!ret) ret = bk7258_preferences_thinking_get(&enabled);
  if (!ret && enabled != requested) ret = -EIO;
  if (!ret) bkagent_cloud_set_thinking(enabled);
  syslog(ret ? LOG_WARNING : LOG_INFO,
  ...
  struct bkcontrol_status_s *status)
{
  ...
  if (!ret) ret = bk7258_preferences_wake_threshold_get(&observed);
  if (!ret && observed != record[7]) ret = -EIO;
  if (!ret) ret = bk7258_agent_trigger_threshold_set(observed);
  syslog(ret ? LOG_WARNING : LOG_INFO,
...
static int product_install_eyes(const uint8_t *record, size_t size)
{
  ...
  char scheme[8];
  char path[256];
  struct url_s url = {.scheme = scheme, .schemelen = sizeof(scheme),
  ...
  struct bkvoice_tls_config_s tls = {.server_ca = &download->ca,
  ...
  if (!ret) ret = bkvoice_tls_initialize(&download->tls, &tls);
  if (!ret) ret = bkcloud_http_get(&download->http, &download->endpoint,
      download->source.url, bkvoice_tls_ops(), &download->tls,
      bkvoice_config_now_ms(NULL) + 8000, download->data, sizeof(download->data));
  uint8_t digest[32];
  if (!ret && download->http.received < 128) ret = -EBADMSG;
  ...
  struct bkcontrol_status_s *status)
{
  ...
  if (kind == BKCONTROL_CONFIG_EYE_PACK) {
    if (command == BKCONTROL_CONFIG_READ) {
      ...
      if (ret) return ret;
      uint32_t fields[] = {display.state, (uint32_t)display.last_error,
      ...
      memcpy(wire + 52, display.source_sha256, sizeof(display.source_sha256));
      memcpy(wire + 84, display.expression, sizeof(display.expression));
      if (offset >= sizeof(wire) || (offset & 15u)) return -ERANGE;
    ...
    if (command != BKCONTROL_CONFIG_BEGIN && command != BKCONTROL_CONFIG_APPLY)
      return -EINVAL;
    if (size < 44 || size > BKCONTROL_OTA_RECORD_MAX) return -EMSGSIZE;
                                     ...
                                     const struct bkcloud_models_s *models)
{
  ...
  if (!ret) ret = models ?
    bkagent_cloud_configure_models(trust, trust_size, cloud, cloud_size, models) :
  ...
  if (!ret)
    {
      ...
      if (!backend) ret = -ENOTSUP;
      if (!ret) (void)claw_config_get("asr_backend", asr, sizeof(asr));
      ...
      if (!ret) {
        ...
        if (!ret && (device_tts ?
            caps.location != VOICE_TTS_LOCATION_DEVICE || caps.needs_network :
            caps.location != VOICE_TTS_LOCATION_REMOTE || !caps.needs_network))
          ret = -EPERM;
      }
      syslog(ret ? LOG_WARNING : LOG_INFO,
...
static int bk7258_agent_config_task(int argc, FAR char *argv[])
{
  ...
  while (1) {
    ...
    if (events & 2) {
       ...
       * product interaction boundary. One admitted wake keeps using official
       * automatic turns until the official endpoint or cancel outcome exits. */
      voice_action = voice_interaction_active &&
                     !product_voice_result_exits_interaction(voice_turn_result) ?
    ...
    if (voice_action != VOICE_ACTION_NONE && now >= voice_action_at) {
      ...
      syslog(action ? LOG_WARNING : LOG_INFO,
             "BKVOICE turn complete=%d interaction=%s next=%s result=%d\n",
             voice_turn_result,
    ...
    if (!atomic_load(&g_agent_core_ready) ||
        !atomic_load(&g_voice_initialized) ||
        !atomic_load(&g_agent_ready)) continue;
    ...
    if ((!g_trigger_started || preferences_pending) &&
      ...
      int threshold_ret = bk7258_preferences_wake_threshold_get(&threshold);
      if (!threshold_ret) bk7258_agent_trigger_threshold_set(threshold);
      ...
      if (!thinking_ret) bkagent_cloud_set_thinking(thinking);
...
static const char g_product_skill_device_assistant[] =
  ...
  "\n"
  "## When to use\n"
  "When the user asks about battery, volume, mood/persona or what the device "
  "is doing, or asks to change volume, mood, eyes or vibration.\n"
  "\n"
  "## How to use\n"
  "1. Volume / mood / battery: call device_status. Battery percentage is "
  "unavailable; never estimate it from voltage.\n"
  "2. Motion: call device_motion for fresh three-axis acceleration. It is an "
  "accelerometer, not a gyroscope.\n"
  "3. Changes: call device_control with action volume (0-100), mood (gentle/"
  "playful/quiet/serious/tsundere_lite), vibrate (1-100 ms) or eyes (neutral/"
  "happy/shy/sad/surprised/thinking/listening/speaking/sleepy). Every repeat "
  "request needs a fresh tool call; report errors honestly.\n"
  "\n"
  "## Example\n"
...
#endif
#ifdef CONFIG_BK7258_VISION_SERVICE
  if (!ret) bk7258_agent_vision_register();
#endif
      ...
      if (!strcmp(message.channel, AGENT_CHAN_VOICE) && message.content)
        {
          ret = message.request_complete ?
                voice_channel_speak_reply(message.request_id, message.content) :
