    ...
    def test_http_adapter(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            subprocess.run([
                ...
                '-DCMAKE_C_FLAGS=-Wno-error=missing-prototypes'], check=True)
            subprocess.run(['cmake', '--build', str(build), '--parallel', '2'],
                           check=True)
            subprocess.run([
                'cc', '-std=gnu11', '-D_GNU_SOURCE', '-pthread', '-Wall', '-Wextra',
                '-Werror', '-Wno-unused-parameter', '-I', str(temp),
                '-I', str(ROOT / 'tests/host/bk7258/mocks'), '-I', str(APPS / 'include'),
                ...
                '-L', str(build / 'library'), '-lmbedcrypto', '-o', str(executable)],
                check=True)
