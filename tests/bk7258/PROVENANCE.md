...
本记录覆盖 `tests/bk7258/` 下 Git 已跟踪的 `*.c`、`*.h`、`*.py`、`*.sh`
源码。2026-08-28 逐文件复核共 160 个，全部标记
`SPDX-License-Identifier: Apache-2.0`。仓库级许可证全文见
[`../../LICENSE`](../../LICENSE)。
...
SDK 版本来源由仓库根目录
[`contest2026_135_yongwangzhiqian.xml`](../../contest2026_135_yongwangzhiqian.xml)
固定。NuttX 由 [`openvela.xml`](../../openvela.xml) 纳入工作区，测试仅针对当前工作区的
公开 ABI 编译。

## 生成物边界

`tests/bk7258/build/` 下的临时源码、分区头、目标文件和可执行文件由测试入口生成且不
