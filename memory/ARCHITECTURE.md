...
| BK7258 initialization layers | `boards/bk7258/common/src/bk7258_platform.c` owns mandatory SDK/IPC/PM/AP lifetime initialization, `bk7258_bringup.c` owns application-facing procfs/MTD/filesystem registration, and the selected physical-board hook owns attached LCD/touch/camera validation and registration. `board_late_initialize()` and `board_app_initialize()` are thin NuttX entry points. |
