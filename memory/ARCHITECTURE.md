...
|---|---|
| BK7258 logical board and physical variants | `chips/bk7258` owns SoC mechanisms, `boards/bk7258/common` owns shared board policy, and `boards/bk7258/{t5ai_core,t5_board,aidk_ai_toy}` own physical bindings. Manifest linkfiles project these into `vendor/beken`; storage and boot algorithms contain no board-name branch. See [ADR-023](decisions/ADR-023-bk7258-platform-board-variants.md) and [ADR-033](decisions/ADR-033-openvela-native-beken-layout-and-gcc10.md). |
| BK7258 build and package boundary | `tools/bk7258/bk7258.py build|sdk|release|package|verify|toolchain` is the only public tool. Internal domains own build orchestration, SDK transactions, partition layout, image bytes, deterministic `.bkpack`, trust and the content-locked Arm GCC10 prebuilt. CP/AP use official `build.sh`; project BL1/BL2 build out of tree. There is no dual-image shell, postbuild hook, framework, executor or compatibility alias. |
| BK7258 build profiles | Each explicit config directory contains `defconfig` and `profile.conf`; the latter owns board, role, compatibility and SDK profile. Each physical board's strict `openvela.conf` selects its normal CP/AP pair and partition for `build --board`; special profiles use the explicit form. MCUboot derives build-local defconfig overlays instead of tracked boot-mode copies. Signing and package publication consume the hash-bound build manifest and never branch on a physical-board name. See [ADR-028](decisions/ADR-028-bk7258-single-cli-data-owned-build.md). |
| BK7258 initialization layers | `chips/bk7258/cp/bk7258_cp_platform.c` owns the complete CP SoC stage table, order, prerequisites, first-error propagation and cached status; the AP equivalent is chip-owned as well. The thin board late bridge supplies immutable storage/GPIO facts and reports only explicit OTA/device checkpoint results. `bk7258_bringup.c` owns application-facing procfs/MTD/filesystem registration, and selected physical-board hooks own attached LCD/touch/camera validation and registration. `board_late_initialize()` and `board_app_initialize()` remain NuttX board entry points. |
