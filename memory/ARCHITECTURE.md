...
| BK7258 build and package boundary | Classic Make and CMake retain ordinary NuttX/OpenVela archives and role binaries as independently visible inputs. The existing signed dual-image packer adds the board-owned BL1/BL2/CP/AP layout, signatures and Flash manifests without redefining the architecture/chip/board layers. The current `.bkpack` framework is a strict metadata-only contract and the existing executable package is a directory; a single portable `.bkpack` container and cross-platform flash transport are not yet implemented. |
