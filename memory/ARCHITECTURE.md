...
| BK7258 build and package boundary | Classic Make and CMake retain ordinary NuttX/openvela `libarch.a` and `libboards.a` archives. The dual build exposes logical role images as `vela_cp.bin`/`vela_ap.bin`; Beken BL1/BL2 and CRC-expanded Flash files remain vendor boot/download artifacts. Signed profiles additionally emit a deterministic, ZIP-compatible `firmware.bkpack` containing the verified payloads, manifest, and a Windows manual-download plan. This is an additive Beken delivery format, not an official openvela artifact and not a cross-platform loader. |
| BK7258 build profiles | The current `board/bk7258/configs/<profile>` tree is the legacy/as-is compatibility surface frozen by P0. Its `defconfig` and adjacent `profile.conf` remain unchanged until migration equivalence is proven. ADR-024 describes that historical shape; its target architecture is superseded by the reviewed direction in [ADR-026](decisions/ADR-026-bk7258-platform-v2-configuration-and-isolation.md). |
