...
见 [637 验收摘要](docs/verification/bk7258/2026-09-20-shaniu-637-full-image.md)。
...
- 635 曾为该轮的最终板测包；当日稍后完成 637 的构建与实机验证，635 记录按版本
  保留、不再追加结论。公开源码可重建实现，不承诺更换用户身份、签名或私有音频
  后仍产生同一全片哈希。旧 build manifest 未单列 Agent 输入摘要，因此本次差分
  对应证明不冒充 635/637 完整二进制逐字节干净复现。
