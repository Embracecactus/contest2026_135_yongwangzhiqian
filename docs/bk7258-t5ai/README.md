...
> N9 CP/AP RPTUN/OpenAMP/RPMsg 当前计划（CURRENT，static-only）：[`nuttx-port/prompts/09-n9-rptun-rpmsg.md`](nuttx-port/prompts/09-n9-rptun-rpmsg.md)
...
| **NuttX Stage N9（CP/AP RPTUN/OpenAMP/RPMsg）** | **CURRENT：`static-only` planning（2026-07-31）**。保留 AP native SMP，在 CP 与整个 AP cluster 之间建立一套 RPTUN link；AP mailbox IRQ→semaphore→logical CPU0 pinned worker；32 KiB carveout 仍是待 N9-A map/static-assert 冻结的 candidate。尚未实现、构建或板测 |
...
| 当前门禁 | N8-D1 仍是 latest verified AP baseline；N9 当前只完成计划。先通过 resource-role 源码核查与 carveout/linker gate，再写 mailbox/RPTUN；官方 NuttX 源码零改动 |
...
- [nuttx-port/prompts/09-n9-rptun-rpmsg.md](nuttx-port/prompts/09-n9-rptun-rpmsg.md) —— Stage N9 CURRENT 计划：SMP/BMP/OpenAMP 选型、shared-memory/worker/role 设计、分阶段实现与验收门禁
...
- [nuttx-port/n5-flash-filesystem.md](nuttx-port/n5-flash-filesystem.md) —— Stage N5 flash filesystem
  ...
  - **Latest AP Stage handoff：** [nuttx-port/n8-d1-smp-lifecycle-quiesce.md](nuttx-port/n8-d1-smp-lifecycle-quiesce.md)（`board-verified`，2026-07-30）；**Current maintenance result：** [nuttx-port/n8-cold-reset-resolution-report.md](nuttx-port/n8-cold-reset-resolution-report.md)（cold/AP 修复完成，warm/reset 各 3/3）
