...
> N8 physical cold-reset NSH 静默问题完整复盘（原 UART blocker 首次板端修复通过）：[`nuttx-port/n8-cold-reset-resolution-report.md`](nuttx-port/n8-cold-reset-resolution-report.md)
...
| **N8 physical cold-reset 启动覆盖** | ✅ 首次真实板端闭环：手动 RESET 得到 `BClk→S0/U0/G1/U1→U2..U5→C0..C3→A0..A7/F1/F2→C4..C8→NSH`；GPIO-before-UART 与 post-SDK console restore 共同修复原“GPIO 日志后永久静默”问题。当前 `ap_smp_bidir` 仍在默认 READY timeout 后走 deterministic cleanup，但 CP fail-open 到 NSH 正常；physical RESET×3 / power-cycle×3 重复性矩阵待完成 |
...
| 当前门禁 | N8-D1 仍是最新 AP Stage `board-verified` baseline；另已完成 physical cold-reset NSH 修复首次板端闭环。当前收口任务是 cold-reset 重复性矩阵、临时 checkpoint 清理和独立 AP READY timeout follow-up；不得把 AP timeout 重新归因于 UART，也不得把 D1 描述为 CPU hot-unplug |
| Tier-2 bootloader（OTA / A-B failover） | 后续，未编号 |
| 多核后续 | CPU1 NuttX UP、CPU2 probe、secondary bootstrap、双向 IPI、scheduler-online IDLE、CPU1-affinity one-task、single/8-cycle semaphore wake、C5 bidirectional pingpong、C6 dual CPU1 local handoff 与 C7 controlled migration 均已板端通过；C5 exact attribution 为 `+9/+8/+17`，C6 为 `+3/+0/+3`，C7 为 `+4/+4/+8`。C8 timed wake 已以 `+9/+0/+9` 完成板端闭环；D1 bounded quiesce/resume 也已以 `+1/+0/+1` 完成板端闭环，且保持 CPU1 online=`0x3`。默认 CPU1 placement、非受控 migration/load balancing、运行时可变/无限循环、RPTUN/RPMsg 实现、Wi-Fi/BLE 服务层仍未启用或验证 |
...
- [nuttx-port/n8-cold-reset-resolution-report.md](nuttx-port/n8-cold-reset-resolution-report.md) —— physical cold-reset NSH 静默问题的完整复盘：路标定位、两层 UART 根因、构建入包、自动下载和最终手动 cold trace
...
- [nuttx-port/n5-flash-filesystem.md](nuttx-port/n5-flash-filesystem.md) —— Stage N5 flash filesystem
  ...
  - **Latest AP Stage handoff：** [nuttx-port/n8-d1-smp-lifecycle-quiesce.md](nuttx-port/n8-d1-smp-lifecycle-quiesce.md)（`board-verified`，2026-07-30）；**Current maintenance handoff：** [nuttx-port/n8-cold-reset-resolution-report.md](nuttx-port/n8-cold-reset-resolution-report.md)（cold NSH first-pass verified，重复性矩阵待完成）
