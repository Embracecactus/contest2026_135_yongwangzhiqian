...
本文把 openvela 中文官网 dev-ai-contest-2026 版本的目录逐项映射到
BK7258 / T5-AI 当前交付状态，用于回答三个长期问题：
...
- 审计日期：2026-08-27
- 集成起点：`e9ba46119bac46dfc4903a9ab2b4ff9891fa46c9`；本表同时审计当前 P0 工作区差异
...
|---|---|---|---|
| [1443 新平台适配](https://doc.openvela.com/document?id=1443&version=dev-ai-contest-2026&language=cn) | ✅ | BK7258 CP/AP/CPU2、板级层、SDK wrapper、manifest 和统一构建入口均为当前交付；见 [移植报告](porting-report.md) | 只做回归维护 |
| [1444 中断适配](https://doc.openvela.com/document?id=1444&version=dev-ai-contest-2026&language=cn)、[1468 中断系统](https://doc.openvela.com/document?id=1468&version=dev-ai-contest-2026&language=cn) | ✅ | 64 路 IRQ、RAM vector、SDK IRQ bridge、GPIO IRQ、SMP IPI 已实现；Trace 实板看到 IRQ entry/exit | 保持 IRQ、GPIO 和 SMP 回归 |
| [1445 Vendor 代码仓](https://doc.openvela.com/document?id=1445&version=dev-ai-contest-2026&language=cn) | ✅ | 团队代码只落在 contest 仓，通过 manifest/linkfile 消费；NuttX 和 SDK 保持上游/只读边界 | 公共 NuttX 改动继续单独提 PR |
| [1448 CMake](https://doc.openvela.com/document?id=1448&version=dev-ai-contest-2026&language=cn)、[1449 CMake 维护](https://doc.openvela.com/document?id=1449&version=dev-ai-contest-2026&language=cn)、[1450 Kconfig](https://doc.openvela.com/document?id=1450&version=dev-ai-contest-2026&language=cn)、[1451 Makefile](https://doc.openvela.com/document?id=1451&version=dev-ai-contest-2026&language=cn) | ⚪ / 🟡 | Kconfig、Classic Make 和仓内统一 CLI 已用于交付；完整 CMake/Make 语义等价仍未宣称 | Classic Make 为发布权威；若比赛要求 CMake，再单列收口 |
...
| [1456 启动流程](https://doc.openvela.com/document?id=1456&version=dev-ai-contest-2026&language=cn) | ✅ T5-Board / 🟡 其他板型 | 当前代码和 clean build 支持三块板的 BL1 → MCUboot BL2 → 同槽签名 CP/AP、ROMFS 双脚本和 final-init；T5-Board 当前签名整机已有 [Agent 验收](../../progress/verification/2026-08-24-bk7258-openvela-agent-ap.md)，AIDK/T5AI-Core 当前只按构建/包验证计，不借用 T5-Board 实板结论；开发期链路边界见 [启动链证据矩阵](../../progress/verification/2026-08-07-bk7258-boot-chain-evidence-matrix.md) | 各板型分别记录 clean/factory/warm/cold；未跑的板型保持 🟡 |
...
| [1508 外设总览](https://doc.openvela.com/document?id=1508&version=dev-ai-contest-2026&language=cn) | ⚪ | 作为能力清单；具体状态以本表和 [驱动缺口记录](../../progress/verification/2026-08-19-bk7258-missing-drivers.md) 为准 | 不单独实现 |
...
| [1632 硬件性能](https://doc.openvela.com/document?id=1632&version=dev-ai-contest-2026&language=cn) | ✅ 当前基线 | `t5_board_cp_perf` generation 145 以 `-O3`、CPU0 有效 160 MHz、AP/Wi-Fi/RPTUN/WDT/Trace/监控关闭运行；CoreMark/Ramspeed/Whetstone 各 10 次统计与重启通过 | 配置、频率或工具版本变化时重建基线 |
...
3. 新建一份 progress/verification/YYYY-MM-DD-*.md 保存可复现命令、产物身份、
   实际结果、失败边界和未覆盖项；
4. 在 [progress/CURRENT.md](../../progress/CURRENT.md) 只保留当前目标、已接受状态和
   下一步，不把长日志复制进去；
...
这样本文负责“官网能力全景图”，CURRENT.md 负责“现在正在做什么”，verification
文档负责“为什么可以相信这个结论”，三者职责不会混在一起。
