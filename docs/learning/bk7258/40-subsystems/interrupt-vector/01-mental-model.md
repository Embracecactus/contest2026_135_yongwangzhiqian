...
本篇解释 BK7258 的中断如何从硬件到达 NuttX 的中断处理函数。这个链路涉及四层，每一层都有自己的编号系统。理解这些编号之间的映射关系，是排查"中断不触发"问题的基础。板端标签均为来源日期对应的教学快照；当前事实以源码、维护配置和 `docs/verification/bk7258/` 中匹配板型的记录为准。
