...
> 官方来源：[doc.openvela.com](https://doc.openvela.com/document?id=1739&version=dev-ai-contest-2026&language=cn)  
> 版本：dev-ai-contest-2026（中文）  
> 官方更新时间：2026-06-09 19:52:59  
> 本地拉取日期：2026-08-28

\[ [English](https://github.com/open-vela/docs/tree/dev-ai-contest-2026//https:/github.com/open-vela/docs/blob/dev/en/faq/devoloper_tech_faq.md) | 简体中文 \]

## 一、社区与通用

### 1\. 遇到技术问题或 Bug 怎么办？

如果是技术类问题，请在 [Issue 页面](https://github.com/open-vela//docs/https:/github.com/open-vela/docs/blob/docs/issues) 提交。
...
是的，社区设有贡献激励机制。详细的奖励规则和说明请参阅[贡献奖励说明](https://github.com/open-vela//docs/https:/github.com/open-vela/docs/blob/docs/issues)。
...
系统支持增量编译。
  ...
  - 如果您只修改了 <span class="reference">.c</span> 或 <span class="reference">.h</span> 源文件，可以直接追加编译，速度较快。
  - 但如果您修改了 <span class="reference">Kconfig</span> (menuconfig) 配置文件（即打开或关闭了某些功能），为了确保配置生效，建议进行完整的重新编译。
...
### 9\. 代码中看到很多以 NX\_ 开头的函数（如 nx\_read），我应该在应用程序中使用它们吗？

**不建议使用**。

以 <span class="reference">NX\_</span> 开头的函数通常为内核内部使用的系统调用或底层封装。

为了保证代码的规范性和可移植性（openvela 已通过 PNS 52 认证），请务必使用标准的 POSIX 接口（如 <span class="reference">open</span>, <span class="reference">read</span>, <span class="reference">pthread\_create</span>）进行开发。
...
请按顺序排查以下几点：
  ...
  - 确认您是基于 <span class="reference">trunk</span> 分支下载代码。
...
因为 <span class="reference">goldfish arm64</span> 等基础配置主要用于验证 CPU 架构和内核基础功能，并未默认开启完整的网络桥接或外设支持。

若需验证网络或多媒体功能，建议使用产品形态的配置文件，例如 <span class="reference">smart speaker</span> 或 <span class="reference">ARMv7a Goldfish</span> 的完整配置。
...
请根据代码的通用性决定：
  ...
  - 通用的驱动框架、调度代码或 Bug 修复建议提交至 <span class="reference">nuttx</span> 主目录（如 <span class="reference">drivers</span> 下）。
  - 特定芯片厂商或私有的板级驱动代码，建议存放在 <span class="reference">vendor</span> 目录下。
...
openvela 社区提供了一份现成的 [xTS 测试用例精简集](https://doc.openvela.com/document?id=XXX&version=trunk&language=cn)，涵盖系统内核、驱动 BSP、文件系统、WiFi、蓝牙、音视频等基础能力的标准测试用例，可直接拷贝命令到 nsh 中执行，无需从零编写。
...
目前可用的方案包括：已完成移植的 FFmpeg、系统内置的原生多媒体工具集（请参考 [Sim 环境音频功能开发指南](https://doc.openvela.com/document?id=1433&version=dev-ai-contest-2026&language=cn)），以及 <span class="reference">libx264</span>、<span class="reference">openh264</span>、<span class="reference">libopus</span> 等已移植的开源编解码库。
...
建议开发者直接参考源码目录下的 <span class="reference">apps/external</span> 文件夹。该目录包含大量已移植的第三方库，是理解构建系统和移植方法的最佳实践。
...
相关库通常位于 <span class="reference">apps/netutils</span> 或 <span class="reference">external</span> 目录下，可直接参考源码。
