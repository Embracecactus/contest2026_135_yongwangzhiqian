...
> 官方来源：[doc.openvela.com](https://doc.openvela.com/document?id=1571&version=dev-ai-contest-2026&language=cn)  
> 版本：dev-ai-contest-2026（中文）  
> 官方更新时间：2026-06-09 19:51:29  
> 本地拉取日期：2026-08-28

\[ [English](https://github.com/open-vela/docs/tree/dev-ai-contest-2026//https:/github.com/open-vela/docs/blob/dev/en/device_dev_guide/graphics/VSync.md) | 简体中文 \]
...
2.  渲染器（Render）以 100Hz（周期 10ms）的频率在 Framebuffer 上进行渲染，每次的渲染时间为 8ms。渲染的内容为一个蓝色矩形从左往右做平移动画。
    ...
    ![img](https://vela-open-doc.cnbj1.mi-fds.com/vela-open-doc/1781005751607_001.svg)
...
![img](https://vela-open-doc.cnbj1.mi-fds.com/vela-open-doc/1781005751702_002.svg)

导致画面撕裂的根本原因其实就是内存踩踏。LCD 在读取 Framebuffer 的过程中，渲染器又往 Framebuffer 写入了新的数据，导致屏幕上同时显示了新帧和旧帧。时序如下图所示：

![img](https://vela-open-doc.cnbj1.mi-fds.com/vela-open-doc/1781005751791_003.svg)
...
![img](https://vela-open-doc.cnbj1.mi-fds.com/vela-open-doc/1781005751882_004.svg)
...
![img](https://vela-open-doc.cnbj1.mi-fds.com/vela-open-doc/1781005751986_005.svg)
...
![img](https://vela-open-doc.cnbj1.mi-fds.com/vela-open-doc/1781005752074_006.svg)
...
![img](https://vela-open-doc.cnbj1.mi-fds.com/vela-open-doc/1781005752165_007.svg)

**说明**

在注册中断服务函数时，需要将驱动程序的 <span class="reference">priv</span> 传入 <span class="reference">arg</span>，这样可以避免使用全局变量传递参数，示例代码如下：  
...
在大部分的业务场景中，是基于 [libuv](https://libuv.org/) 进行开发的，这意味着上层不能使用任何诸如 <span class="reference">sem\_wait</span>、<span class="reference">usleep</span> 等同步阻塞等待接口，否则会影响整个事件循环的运行。
...
![img](https://vela-open-doc.cnbj1.mi-fds.com/vela-open-doc/1781005752256_008.svg)

openvela 的 Framebuffer 驱动框架提供了 <span class="reference">poll</span> 所需要的[接口](https://github.com/open-vela//docs/https:/github.com/open-vela/docs/nuttx/blob/dev-ai-contest-2026/drivers/video/fb.c)，用于监控 Framebuffer 是否处于可写状态：  
...
在 Framebuffer 中引入一个队列机制，称为 <span class="reference">panbuf</span> 队列。<span class="reference">panbuf</span> 队列本质上是一个简单的环形缓冲区，里面保存的是渲染完成即将发送的 buffer 信息 <span class="reference">union fb\_paninfo\_u</span>。
...
![img](https://vela-open-doc.cnbj1.mi-fds.com/vela-open-doc/1781005752353_009.svg)

渲染器通过调用 <span class="reference">FBIOPAN\_DISPLAY</span> ioctl 接口向底层的 panbuf 队列推入数据。

对于支持 FB overlay 的 LCD 控制器，使用 <span class="reference">FBIOPAN\_OVERLAY</span> ioctl 接口向 overlay panbuf 队列推送数据。  
...
驱动程序使用 <span class="reference">fb\_remove\_paninfo</span> 函数向上层发起通知，表示该 buffer 已经不再使用，<span class="reference">fb\_remove\_paninfo</span> 会主动去通知当前阻塞等待绘制的线程。  
...
在新的 panbuf 队列机制下，驱动适配 VSync 需要使用如下 API 接口：
  ...
  - fb\_peek\_paninfo : 读取 panbuf 队列第一帧的信息。

  - fb\_remove\_paninfo : 删除 panbuf 队列第一帧。

  - fb\_paninfo\_count ：获取 panbuf 队列的 paninfo 个数。  
...
使用信号量进行同步，相当于对 Framebuffer 进行加锁操作，渲染器每次开始渲染时都需要拿到锁才能进行绘制，否则就会处于阻塞状态，代码请参见此 [链接](https://github.com/open-vela//docs/https:/github.com/open-vela/docs/nuttx/blob/dev-ai-contest-2026/arch/arm/src/stm32/stm32_ltdc.c)。
