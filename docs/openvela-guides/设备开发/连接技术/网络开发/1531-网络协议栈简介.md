...
> 官方来源：[doc.openvela.com](https://doc.openvela.com/document?id=1531&version=dev-ai-contest-2026&language=cn)  
> 版本：dev-ai-contest-2026（中文）  
> 官方更新时间：2026-06-09 19:51:07  
> 本地拉取日期：2026-08-28

\[ [English](https://github.com/open-vela/docs/tree/dev-ai-contest-2026//https:/github.com/open-vela/docs/blob/dev/en/device_dev_guide/connection/network/protocol_stack/NetProtocolStackIntro.md) | 简体中文 \]
...
![img](https://vela-open-doc.cnbj1.mi-fds.com/vela-open-doc/1781005746433_001.svg)
...
openvela 网络协议栈支持在 IPv4 和 IPv6 上运行 TCP 协议，并提供以下功能：
  ...
  - 标准 socket 接口支持：
      - 提供 <span class="reference">bind</span>、<span class="reference">listen</span>、<span class="reference">connect</span>、<span class="reference">accept</span>、<span class="reference">send</span>、<span class="reference">recv</span>、<span class="reference">shutdown</span>、<span class="reference">poll</span> 等标准 POSIX Socket 接口。
  - TCP 特性支持：
      - Backlog：支持连接队列的管理。
...
### UDP 能力
  ...
  - 标准 Socket 接口支持：
      - 提供 <span class="reference">bind</span>、<span class="reference">listen</span>、<span class="reference">connect</span>、<span class="reference">send</span>、<span class="reference">recv</span>、<span class="reference">poll</span> 等标准 POSIX Socket 接口。
  - UDP 特性支持：
...
![img](https://vela-open-doc.cnbj1.mi-fds.com/vela-open-doc/1781005746537_002.png)

### 工作原理

  - Usrsock Client 侧：
      - 用户态程序的所有套接字操作（如 <span class="reference">send</span>、<span class="reference">recv</span> 等）都会通过 RPMsg 等代理方式转发到 Server 侧。
  - Usrsock Server 侧：
      - 负责实际处理套接字操作。
