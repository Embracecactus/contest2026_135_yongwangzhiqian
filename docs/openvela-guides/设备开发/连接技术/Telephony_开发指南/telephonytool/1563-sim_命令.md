...
> 官方来源：[doc.openvela.com](https://doc.openvela.com/document?id=1563&version=dev-ai-contest-2026&language=cn)  
> 版本：dev-ai-contest-2026（中文）  
> 官方更新时间：2026-06-09 19:51:25  
> 本地拉取日期：2026-08-28

\[ [English](https://github.com/open-vela/docs/tree/dev-ai-contest-2026//https:/github.com/open-vela/docs/blob/dev/en/device_dev_guide/connection/telephony/telephonytool/sim.md) | 简体中文 \]
...
<span class="reference">listen-sim</span> 命令用于注册监听与 SIM 卡相关的事件。
...
### 命令格式
  ...
  - event\_id: 要监听的事件 ID，支持以下事件：
      - <span class="reference">MSG\_SIM\_STATE\_CHANGE\_IND</span>: SIM 状态变更事件。
      - <span class="reference">MSG\_SIM\_UICC\_APP\_ENABLED\_CHANGE\_IND</span>: SIM UICC 应用启用状态变更事件。
      - <span class="reference">MSG\_SIM\_ICCID\_CHANGE\_IND</span>: SIM ICCID（集成电路卡标识符）变更事件。
...
<span class="reference">unlisten-sim</span> 命令用于取消监听与 SIM 卡相关的事件。

### 命令格式

    unlisten-sim [watch_id]

  - watch\_id: 对应 <span class="reference">listen-sim</span> 命令返回的监听 ID，用于标识需要取消的监听事件。
...
<span class="reference">has-icc</span> 命令用于查询指定插槽中是否存在 ICC（Integrated Circuit Card，集成电路卡）。
...
<span class="reference">get-sim-state</span> 命令用于获取指定插槽中 SIM 卡的状态。
...
<span class="reference">get-iccid</span> 命令用于获取指定插槽中 SIM 卡的 ICCID（Integrated Circuit Card Identifier，集成电路卡标识符）信息。
...
<span class="reference">get-sim-operator</span> 命令用于获取指定插槽中 SIM 卡的 PLMN（Public Land Mobile Network，公共陆地移动网络）信息。PLMN 是由运营商的国家代码（MCC）和网络代码（MNC）组成的标识符，用于标识 SIM 卡所属的运营商。
...
<span class="reference">get-sim-operator-name</span> 命令用于获取指定插槽中 SIM 卡所属运营商的名称（SPN，Service Provider Name）。
...
<span class="reference">get-sim-subscriber-id</span> 命令用于获取指定插槽中 SIM 卡的订阅者标识（Subscriber ID）。
...
<span class="reference">change-pin</span> 命令用于修改指定插槽中 SIM 卡的 PIN（Personal Identification Number，个人识别码）。
...
### 命令格式
  ...
  - slot\_id: 设置要查询的插槽，目前仅支持 <span class="reference">0</span>。
  - pin\_type: PIN 码类型，可选值为 <span class="reference">pin</span> 或 <span class="reference">pin2</span>。
  - old\_pin: 当前的 PIN 码值。
  - new\_pin: 要设置的新 PIN 码值。
...
<span class="reference">enter-pin</span> 命令用于验证指定插槽中 SIM 卡的 PIN（Personal Identification Number，个人识别码）。
...
<span class="reference">reset-pin</span> 命令用于通过 PUK（Personal Unblocking Key，个人解锁密钥）重置指定插槽中 SIM 卡的 PIN（Personal Identification Number，个人识别码）。
...
### 命令格式
  ...
  - slot\_id: 设置要查询的插槽，目前仅支持 <span class="reference">0</span>。
  - puk\_type: PUK 码类型，可选值为 <span class="reference">puk</span> 或 <span class="reference">puk2</span>。
...
<span class="reference">lock-pin</span> 命令用于激活 SIM 卡的 PIN 锁功能，确保 SIM 卡的安全性。
...
<span class="reference">unlock-pin</span> 命令用于去激活 SIM 卡的 PIN 锁功能，解除对 SIM 卡的 PIN 验证要求。
...
<span class="reference">open-logical-channel</span> 命令用于打开逻辑通道，以便对 SIM 卡进行读写操作。
...
### 命令格式
  ...
  - slot\_id: 设置要查询的插槽，目前仅支持 <span class="reference">0</span>。
  - aid\_str: 应用标识符（AID，Application Identifier）字符串，用于指定目标应用。
...
##### 输出信息
  ...
  - session id: 返回的 <span class="reference">session id</span> 表示逻辑通道的会话标识符，用于后续的读写操作。

## 15、close-logical-channel

### 命令说明

<span class="reference">close-logical-channel</span> 命令用于关闭指定的逻辑通道，释放与 SIM 卡的会话资源。
...
### 命令格式
  ...
  - slot\_id: 设置要查询的插槽，目前仅支持 <span class="reference">0</span>。
  - session\_id: 要关闭的逻辑通道的会话标识符（session ID）。
...
##### 输出信息
  ...
  - session\_id: 输出信息中的 <span class="reference">session id</span> 表示已成功关闭的逻辑通道会话标识符。

## 16、transmit-apdu-basic-channel

### 命令说明

<span class="reference">transmit-apdu-logical-channel</span> 命令用于通过逻辑通道向 SIM 卡发送 APDU（Application Protocol Data Unit，应用协议数据单元）命令。
...
### 命令格式
  ...
  - slot\_id: 设置要查询的插槽，目前仅支持 <span class="reference">0</span>。
  - session\_id: 逻辑通道的会话标识符（session ID）。
  - pdu: 要发送的 APDU 数据内容。
  - len: APDU 数据的字节长度。
...
<span class="reference">transmit-apdu-basic-channel</span> 命令用于通过基本通道向 SIM 卡发送 APDU（Application Protocol Data Unit，应用协议数据单元）命令。
...
<span class="reference">get-uicc-enablement</span> 命令用于获取 UICC（Universal Integrated Circuit Card，通用集成电路卡）应用的启用状态。
...
##### 输出信息
  ...
  - state:
      - <span class="reference">0</span> 表示 UICC 应用未启用。
      - <span class="reference">1</span> 表示 UICC 应用已启用。

## 19、set-uicc-enablement

### 命令说明

<span class="reference">set-uicc-enablement</span> 命令用于设置 UICC（Universal Integrated Circuit Card，通用集成电路卡）应用的启用或禁用状态。
...
### 命令格式
  ...
  - state: 指定 UICC 应用的目标状态：
      - <span class="reference">0</span>: 禁用 UICC 应用。
      - <span class="reference">1</span>: 启用 UICC 应用。
