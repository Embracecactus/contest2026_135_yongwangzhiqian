...
> 官方来源：[doc.openvela.com](https://doc.openvela.com/document?id=1606&version=dev-ai-contest-2026&language=cn)  
> 版本：dev-ai-contest-2026（中文）  
> 官方更新时间：2026-06-09 19:51:47  
> 本地拉取日期：2026-08-28

\[ [English](https://github.com/open-vela/docs/tree/dev-ai-contest-2026//https:/github.com/open-vela/docs/blob/dev/en/edge_ai_dev/tflite_micro_integration.md) | 简体中文 \]
...
| **Tensor** **Arena**       | 一块预先分配的大型连续内存区域。TFLM 不使用 <span class="reference">malloc/free</span>，而是将模型输入、输出及中间计算数据全部放置在此区域。 | 决定了系统能运行多大的模型，需根据 SRAM 大小谨慎配置。                                                    |
...
| **Op** **Resolver**        | 算子解析器。用于在运行时查找并注册模型所需的算子实现。                                                                    | 推荐使用 <span class="reference">MicroMutableOpResolver</span> 按需注册，避免引入无用代码导致固件体积膨胀。 |
| **Quantization (量化)**      | 将 32 位浮点数转换为 8 位整数的技术，旨在减少模型体积并加速计算。                                                           | openvela 推荐运行 <span class="reference">int8</span> 量化模型以获得最佳性能。                    |
...
开发者若需自定义算子或封装硬件加速驱动，需遵循 <span class="reference">TfLiteRegistration</span> 接口定义：  
...
在 <span class="reference">tensorflow/lite/micro/kernels/</span> 目录下，代码按算子功能组织：  
...
NuttX 提供了完整的 POSIX 标准支持，TFLite Micro 的推理任务通常封装在标准的 <span class="reference">pthread</span> 或 NuttX 任务（Task）中。
...
虽然可以使用 <span class="reference">malloc</span> 动态申请 Arena，但强烈建议采用静态分配。  
