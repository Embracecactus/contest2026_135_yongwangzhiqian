...
- F01–F12 整改的 13 个提交已合入官方主仓 `dev-ai-contest-2026`
  ...
  之后的 636/637 提交（`019a449e`、`faab4493`、`7d667565`）也已合入。
  后续开发与复现以主仓该分支为基线，不再从 fork 交付分支继续。
...
- 烧录由用户手动完成（本机没有该镜像的传输日志）；回贴串口日志出现
  ...
  同一观察窗口内有一次 ASR 请求瞬时 `ret=-5`、随后请求成功（是否跨越待机/再次
  唤醒以完整记录为准，摘录不足以证明交互边界）。637/638 均未重测
...
（本节为 637 之前的当日状态，非当前结论。）

- 原交付 PR [#116](https://github.com/open-vela/contest2026_135_yongwangzhiqian/pull/116)
  已合入官方 `dev-ai-contest-2026@73bc1fda`；后续修复从该基线另建分支，
  不把主仓合入等同 Agent 依赖合入或比赛网站正式提交。
- T5 COM3 完成两次不同源码输入的受控下载，定位并修复 GT9xx/LVGL 输入契约
  不匹配；`c6976458` 界面初始化实板通过，TF 无响应、人工触摸和录音落卡仍待确认。
  不改公共框架/SDK，不继承历史录音成功给新包。见[小海豚记录](dolphin-master-plan.md)。
- AIDK COM8 保留 635，通过 shell reset 验证配置/模型阈值/眼睛资源恢复与只读
  外设状态；未重新烧录、未重新认领。BT 早期告警仍在，完整语音/App/OTA 未重验。
- 8 个开发 Skill 规范检查通过；两个硬件工具已实际使用。手机未连 ADB、声学
  播放路由未确认，未将缺少条件的工作流标为通过。见[Skill 覆盖](shaniu-skill-capability-map.md#2026-09-20-补验覆盖)。

## 2026-09-20 代码与比赛材料发布收口

（本节为 637 之前的当日状态；其中的“635 是最终包”已被上方的 637 结论取代。）
...
- 合并作者另一会话的四张实物照片、海报和 18 页 PPT 成品，用于统一比赛 ZIP；
  ...
  当轮结论是 635 为最终包、只做源代码/依赖/文档交付；随后用户重新授权，
  当日又完成 637 的构建与实机验证，见顶部小节。
- 635 原构建输入 570 项与封存摘要一致；公开快照 564 项的共同输入逐字节一致，
  差集仅为 3 组已退役 AIDK 验证配置的 6 个文件，未删功能实现或修改产品路径。
  Agent 差分也与 fork 提交一致；详细摘要见来源记录。
- 隔离构建发现 T5-Board 的可选录音仍选中已退役兼容 ABI。`c10a7668` 只改
  该功能的团队 Kconfig/板级选择，并关闭录音；这不是录音功能的完整复现。
  **同日更正**：用户确认小海豚录音保存 SD 已实测，已有 `0.1.0+13` 的正常
  路径验收记录；此前由本次构建差异推断为“功能未完成”不准确。
  后续 `8de0ae78` 已恢复误删的 Dolphin 专用 NuttX 录音接线及默认开关，
  采集/WAV 逻辑未改，T5 构建、既有录音主机检查和 ELF 核对通过；未重新板测。
  详见 [小海豚记录](dolphin-master-plan.md)。635 不变。
- `c10a7668` 已完成三板 CP/AP direct 构建；52 个实际检出依赖与锁定 manifest
  一致，SDK/toolchain 复用经过现役工具哈希验证的缓存，未重编 SDK。详见
  [隔离构建记录与哈希](../../verification/bk7258/2026-09-20-public-source-build.md)。
  这些未签名构建产物不烧录、不覆盖 635，不合并为新实板验收。
- 同一隔离源码的 Android `assembleDebug` 通过，版本 `0.5.23-shaniu-rebind`
  / code 28，APK 8,038,482 B；哈希记入上述构建记录。未安装或重测手机，
  不把主机编译结果用于扩展 634/635 的设备验收边界。
