...
历史集中沉淀日期：2026-09-14。下方历史来源覆盖当日可访问对话、工作树、
...
2026-09-18 起，7 份通用能力及中文 PR 入口的权威正文随仓库保存在
`.agents/skills/`，必要参考文件一并提供，不再把原作者的用户目录当成交付依赖。
已有硬件 Skill 继续维护在 `tools/bk7258-hil-download/` 与
`tools/windows-hardware-debug/`；跨 Skill 依赖使用同仓相对位置。
用户级安装只是使用方式，不是第二份可独立修改的正文来源。
不打包完整会话、私有设备备份、凭据或原始大日志进入 Skill。

人工开发从 README、板级配置及现有 CLI `--help` 入手即可；Skill 是可选的操作指引，
不参与固件编译、不要求安装 AI 代理，也不增加安装器或新的部署入口。
...
| 官方目标编译接线与来源确认：`frameworks/cmake/` | 同上 | 复用当前官方目标；旧依赖复制、补丁链与构建源替换已退役，不作为本项目使用步骤 |
...
| APK 覆盖安装、真实 BLE、切页与旧期限边界、文件选择器、音量回读、App OTA/取消 | `android-companion-hil` | 复用实机验收 Skill；次数服从当前有限验收范围，不固化历史 20 次要求 |
...
| 中文 PR、实际上游/fork 身份与基线 | `.agents/skills/publish-pr-cn/` | 中文表达层引用同仓相邻 `fork-change-publication`；无固定仓库、作者账号、remote 名称或比赛分支 |
...
| 产品词与实际发音 | 当前模型 metadata、实际加载哈希与产品要求；历史合成用字只是发音适配，不替代实际模型验证 |
...
635 已由用户与 CodeBuddy 验证 `/data/agent/skills/device-assistant.md` 安装、
工具注册和语音/显示；详见 [635 记录](../../verification/bk7258/2026-09-20-shaniu-runtime-skill-635.md)。
它是设备端官方 Agent 消费的 Markdown，与下面的开发期操作 Skill 不同。
App 控制及 OTA 本轮未重测，仍引用 634 记录。

## 历史证据与能力边界

以下 511/v46 记录解释 Skill 的来源，不覆盖后续候选的实际状态。
...
| `publish-pr-cn` | 当时本项目已推送分支需要中文 PR 交付 | 当时尚未覆盖其他仓库 | 该行仅为历史检查；当前入口已去掉项目限制并引用同仓通用流程 |
