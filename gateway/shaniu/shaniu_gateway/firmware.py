...
class FirmwareRelease:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or not _DEVICE_ID.fullmatch(self.device_id)
        ...
        if _generation(self.required_source_version) >= _generation(
            self.target_version
        ):
...
class FirmwareReleases:
        ...
        for device_id in sorted({row.device_id for row in releases}):
            rows = tuple(
                sorted(
                    ...
                    key=lambda row: (
                        _generation(row.target_version),
                        row.target_version,
                        row.manifest_sha256,
                    ),
                )
    ...
    def compatible(
        ...
        try:
            ...
            return ()
        if not isinstance(observed_root_sha256, str) or not _DIGEST.fullmatch(
