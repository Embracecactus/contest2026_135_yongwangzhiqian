...
def decode_ota_report(payload: bytes) -> OtaReport:
    ...
    request_sequence, result, phase, progress, reserved, digest = _OTA_REPORT.unpack(
        payload
    )
    if (
        request_sequence in (0, UINT32_MAX)
        or result > 0
...
def decode_status_report(payload: bytes) -> StatusReport:
    ...
    if (
        ...
        or (percent > 100 and percent != 0xFF)
        or charging not in (0, 1, 2)
        or (state > 5 and state != 0xFF)
        or reserved != 0
    ):
        raise ProtocolError("invalid_status_report")
    ...
    if not unknown_firmware and (
        major == 0xFFFF or minor == 0xFFFF or revision == 0xFFFF or build == UINT32_MAX
    ):
        raise ProtocolError("invalid_status_report")
    ...
    return StatusReport(
        ...
        firmware_version=(
            None if unknown_firmware else f"{major}.{minor}.{revision}+{build}"
        ),
    ...
    def validate(self) -> None:
        ...
        if self.message_type in {
            ...
            if self.turn_id != 0:
                raise ProtocolError("invalid_turn")
            expected = {
                MessageType.VOLUME_GET: 0,
                MessageType.VOLUME_SET: 4,
                MessageType.VOLUME_REPORT: 12,
            }[self.message_type]
            if len(self.payload) != expected:
                raise ProtocolError("invalid_volume_payload")
            if (
                self.message_type is MessageType.VOLUME_SET
                and _WINDOW.unpack(self.payload)[0] > 100
            ):
...
class GatewaySession:
    ...
    def _transition(self, frame: Frame) -> ReceiveResult:
        ...
        if (
            self.state is SessionState.IDLE
            and self._cancelled_turn != 0
    ...
    def make_volume_request(self, percent: int | None = None) -> Frame:
        ...
        if percent is not None and (
            type(percent) is not int or not 0 <= percent <= 100
        ):
            raise ProtocolError("invalid_volume")
        return self._outbound(
            MessageType.VOLUME_GET if percent is None else MessageType.VOLUME_SET,
            turn_id=0,
            payload=b"" if percent is None else _WINDOW.pack(percent),
        )
    ...
    def make_ota_request(self, manifest_sha256: str) -> Frame:
        ...
        if (
            ...
            raise ProtocolError("invalid_ota_request")
        return self._outbound(
            MessageType.OTA_REQUEST, turn_id=0, payload=bytes.fromhex(manifest_sha256)
        )
