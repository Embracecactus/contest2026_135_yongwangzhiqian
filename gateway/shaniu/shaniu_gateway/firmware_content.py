...
def _member_offset(fd: int, info: zipfile.ZipInfo) -> int:
    ...
    if _read_exact(
        fd, info.header_offset + _LOCAL_HEADER.size, name_size
...
def _read_catalog(
    catalog: bytes, release: FirmwareRelease, names: set[str]
) -> tuple[str, str]:
    try:
    ...
    if (
        ...
        or set(value) != fields
        or _canonical(value) != catalog
    ):
        raise FirmwareContentError("noncanonical OTA catalog")
    if (
        value["format"] != "bk7258.ota/2"
        or value["board_family"] != release.board_family
        or value["target"]
        != {
            "board_family": release.board_family,
            "physical_board": release.physical_board,
        }
        or value["layout"]
        != {"identity": release.layout_identity, "sha256": release.layout_sha256}
    ...
    if (
        ...
        or not _DIGEST.fullmatch(package_id)
    ...
    for artifact in ("cp", "ap"):
        ...
        if (
            ...
            or set(entry) != {"uri", "size", "sha256"}
            or not isinstance(entry["uri"], str)
            or not _MEMBER.fullmatch(entry["uri"])
            or type(entry["size"]) is not int
            or not 0 < entry["size"] <= MAX_PACKAGE_SIZE
...
class FirmwareContentStore:
    ...
    def __init__(
        self, releases: FirmwareReleases, packages: tuple[os.PathLike[str] | str, ...]
    ):
        if not isinstance(releases, FirmwareReleases) or not packages:
            raise FirmwareContentError(
                "firmware content requires releases and packages"
            )
    ...
    def _load_one(self, name: str, releases: FirmwareReleases) -> _Package:
        ...
        try:
            ...
            if (
                ...
                or not 0 < info.st_size <= MAX_PACKAGE_SIZE
            ):
            ...
            matches = [
                ...
                if row.package_size_bytes == info.st_size
                and row.package_sha256 == digest
            ]
            if len(matches) != 1:
                raise FirmwareContentError(
                    "package does not uniquely match release registry"
                )
            release = matches[0]
            with os.fdopen(os.dup(fd), "rb") as source:
                with zipfile.ZipFile(source, "r") as archive:
                    ...
                    for row in infos:
                        if (
                            ...
                            or row.file_size != row.compress_size
                            or row.file_size > MAX_PACKAGE_SIZE
                        ):
                            raise FirmwareContentError(
                                "compressed or encrypted package member"
                            )
                    ...
                    if (
                        not 1 <= catalog_info.file_size <= 65536
                        or not 8 <= sig_info.file_size <= 80
                    ):
                    ...
                    if hashlib.sha256(catalog).hexdigest() != release.manifest_sha256:
                        raise FirmwareContentError(
                            "catalog digest does not match release"
                        )
                    cp_name, ap_name = _read_catalog(catalog, release, set(records))
                    members: dict[str, _Member] = {
                        "catalog.json": _Member(
                            "catalog.json",
                            catalog_offset,
                            catalog_info.file_size,
                        ...
                        "catalog.sig": _Member(
                            ...
                            sig_info.file_size,
                            hashlib.sha256(
                                _read_exact(fd, sig_offset, sig_info.file_size)
                            ).hexdigest(),
                    ...
                    for artifact, member_name in (("cp", cp_name), ("ap", ap_name)):
                        ...
                        if (
                            row.file_size != expected["size"]
                            or hashlib.sha256(image).hexdigest() != expected["sha256"]
                        ):
                            raise FirmwareContentError(
                                "OTA image does not match catalog"
                            )
                        members[member_name] = _Member(
                            member_name,
                            offset,
                            row.file_size,
                            expected["sha256"],
    ...
    def process_request(self, protocol, raw_path: str, headers, gateway: GatewayServer):
        ...
        if package is None or name not in package.members:
            return self._response(HTTPStatus.NOT_FOUND)
        tls = (
            protocol.transport.get_extra_info("ssl_object")
            if protocol.transport
            else None
        )
        certificate = tls.getpeercert(binary_form=True) if tls is not None else None
        device_id = (
            gateway.device_bindings.device_for_certificate(
                hashlib.sha256(certificate).hexdigest()
            )
            if certificate is not None and gateway.device_bindings is not None
            else None
        )
        connection = (
            gateway.device_connection(device_id) if device_id is not None else None
        )
        ota = None if connection is None else connection.ota_status
        if (
            device_id != package.release.device_id
            or ota is None
            or ota.manifest_sha256 != manifest
            or ota.phase in {6, 7, 8}
        ):
        ...
        if name in {"catalog.json", "catalog.sig"}:
            ...
            return self._response(
                HTTPStatus.OK,
                _read_exact(package.fd, member.offset, member.size),
