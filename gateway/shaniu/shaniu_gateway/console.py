...
class ConsoleGrant:
    ...
    def __post_init__(self):
        if (
            ...
            or not _SHA.fullmatch(self.token_sha256)
            or not isinstance(self.device_id, str)
            or not _ID.fullmatch(self.device_id)
...
class ConsoleEpochs:
    ...
    def next(self) -> int:
        try:
            ...
            row = self.db.execute(
                "SELECT value FROM console_epoch WHERE id=1"
            ).fetchone()
            if row is None:
    ...
    def persona(self, device_id: str) -> str | None:
        ...
        try:
            row = self.db.execute(
                "SELECT mode FROM console_persona WHERE device_id=?",
                (device_id,),
            ).fetchone()
    ...
    def set_persona(self, device_id: str, mode: str) -> None:
        if (
            ...
            or not _ID.fullmatch(device_id)
...
class ConsoleService:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            gateway.device_bindings is None
            or not grants
            or len(grants) > 256
            or len({g.token_sha256 for g in grants}) != len(grants)
            or any(
                not gateway.device_bindings.contains_device(g.device_id) for g in grants
            )
            or any(
                not gateway.device_bindings.contains_device(device)
        ...
        if (
            ota_transactions is not None
            and gateway_store is not None
        ...
        if (
            gateway_releases.releases
            and gateway_releases.releases != firmware_releases.releases
        ):
    ...
    async def _guard(self, request, handler):
        ...
        grant = next(
            (g for g in self.grants if hmac.compare_digest(g.token_sha256, digest)),
            None,
        )
    ...
    def _state(self, device):
        ...
        if persona not in PERSONA_MODES or not callable(
            getattr(connection.reply_session, "set_persona", None)
        ):
    ...
    def _release_for_transaction(self, row: OtaTransaction) -> FirmwareRelease | None:
        return next(
            (
                release
                for release in self.firmware_releases.releases
                if release.device_id == row.device_id
                and release.manifest_sha256 == row.manifest_sha256
                and release.target_version == row.target_version
            ),
            None,
        )
    ...
    def _snapshot(self, device):
        ...
        phase = {
            ...
            SessionState.UPLINK: "listening",
            SessionState.THINKING: "thinking",
            SessionState.DOWNLINK: "speaking",
        }
        update = dict(
            phase="unknown", target_version=None, progress_percent=0, error=None
        )
        ...
        if (
            ...
            update = dict(
                phase=ota_phase_name,
                target_version=release.target_version,
                progress_percent=row.progress_percent,
                error=(
                    "update_device_error"
                    if row.state in {"failed", "orphaned"}
                    else None
                ),
            )
        else:
            ...
            if (
                ota is None
                or release is None
                or ota.manifest_sha256 != release.manifest_sha256
            ):
                ota = None
        ...
        payload = dict(
            ...
            gateway="online",
            battery_percent=(
                None if device_status is None else device_status.battery_percent
            ),
            charging=(None if device_status is None else device_status.charging),
            firmware_version=(
                None if device_status is None else device_status.firmware_version
            ),
            revision=state.revision,
            turn=(
                "cancel_unconfirmed"
                if state.connection.cancel_confirmation_timed_out
                else (
                    "cancelling"
                    if state.connection.cancel_pending_turn
                    else (
                        "playback_unconfirmed"
                        if state.connection.playback_confirmation_timed_out
                        else (
                            "speaking"
                            if state.connection.playback_pending_turn
            ...
            permissions={
                **{
                    key: "not_granted"
                    for key in ("microphone", "camera", "location", "authorized_voice")
                },
    ...
    async def snapshot(self, request):
        ...
        if (
            state.connection.session.capabilities & CAP_VOLUME
    ...
    async def releases(self, request):
        if (
            set(request.query) != {"generation"}
            or len(request.query.getall("generation")) != 1
            or re.fullmatch(
                r"[1-9][0-9]{0,18}", request.query.get("generation", ""), re.ASCII
            )
            is None
        ):
            raise web.HTTPBadRequest()
    ...
    async def mutate(self, request):
        ...
        if (
            ...
            or value["protocol"] != "console-v1"
            or value["kind"] != "mutation"
            or value["device_id"] != request.match_info["device"]
            or not isinstance(value["request_id"], str)
            or not _ID.fullmatch(value["request_id"])
    ...
    async def _mutate(self, value, state):
        ...
        error = None
        if not (
            value["issued_at_ms"] <= now + 5000
            and value["issued_at_ms"]
            < value["expires_at_ms"]
            <= value["issued_at_ms"] + 60000
            ...
            if (
                set(value["arguments"]) != {"volume_percent"}
            ...
            try:
                await state.connection.request_volume(
                    value["arguments"]["volume_percent"]
                )
            except ProtocolError as failure:
                error = (
                    "gateway_offline"
            ...
            if (
                set(value["arguments"]) != {"release_manifest_sha256"}
                or not isinstance(manifest, str)
                or not _SHA.fullmatch(manifest)
            ...
            matches = tuple(
                release
                for release in self.firmware_releases.compatible(
            ...
            if persisted is not None and persisted.state not in {
                ...
                if (
                    ...
                    or persisted.manifest_sha256 != manifest
                ):
                    error = "revision_conflict"
                else:
                    ...
                    if state.connection.ota_request_sent:
                        ...
                        error = (
                            ...
                            if failure.code == "device_offline"
                                ...
                                if failure.code == "ota_not_supported"
                                else (
                                    "revision_conflict"
                                    if failure.code == "ota_busy"
            ...
            if self.memory_store is None or not self.memory_store.configured(device_id):
                error = "unsupported_operation"
            elif value["operation"] == "permission.configure":
                arguments = value["arguments"]
                if (
                    set(arguments) != {"capability", "state"}
                    or arguments["capability"]
                    not in {
                        "microphone",
                        "camera",
                        "location",
                    ...
                    or arguments["state"] not in {"not_granted", "allowed", "denied"}
                ):
                ...
                if arguments["capability"] != "long_term_memory":
                    ...
                    clear = getattr(
                        state.connection.reply_session, "clear_memory", None
                    )
                ...
                arguments = value["arguments"]
                if set(arguments) != {"scope"} or arguments["scope"] not in {
                    "conversations",
                    "all",
                }:
            ...
            set(value["arguments"]) != {"persona_mode"}
            ...
            try:
                state.connection.reply_session.set_persona(
                    value["arguments"]["persona_mode"]
                )
            except ProviderError:
                raise web.HTTPConflict(text="conversation_unavailable") from None
            try:
                self.epochs.set_persona(
                    value["device_id"], value["arguments"]["persona_mode"]
                )
            except sqlite3.Error:
                try:
        ...
        if value["expires_at_ms"] > now:
            state.receipts[value["request_id"]] = (
                canonical,
                receipt,
    ...
    async def events(self, request):
        if set(request.query) != {"generation", "after_sequence"} or any(
        ...
        if any(
            not re.fullmatch(r"0|[1-9][0-9]{0,18}", request.query[key], re.ASCII)
        ...
        if (
            ...
            or not 0 <= after <= snapshot["sequence"]
        ):
