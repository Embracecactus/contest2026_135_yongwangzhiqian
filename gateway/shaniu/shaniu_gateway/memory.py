...
class MemoryStore:
    ...
    def _open_private_database(path: Path) -> sqlite3.Connection:
        try:
            ...
            for component in parent.parts[1:]:
                ...
                shared_tmp = (
                    current in {Path("/tmp"), Path("/var/tmp")}
            ...
            if parent_info.st_uid != os.geteuid() or parent_info.st_mode & 0o022:
                raise MemoryStateError(
                    "memory state directory must be operator-private"
                )
            fd = os.open(
            ...
            try:
                ...
                if (
                    ...
                    raise MemoryStateError(
                        "memory state must be an owner-private regular file"
                    )
            finally:
                os.close(fd)
    ...
    def permission(self, device_id: str) -> str:
        ...
        try:
            row = self.db.execute(
                "SELECT state FROM memory_devices WHERE device_id=?",
                (device_id,),
            ).fetchone()
            ...
            self.db.execute(
                "SELECT 1 FROM memory_turns WHERE device_id=? LIMIT 1",
                (device_id,),
            ).fetchone()
    ...
    def append(self, device_id: str, transcript: str, reply: str) -> bool:
        ...
        if (
            ...
            or not transcript
            or not reply
            or len(transcript) > MAX_TEXT_CHARS
        ...
        try:
            ...
            self.db.execute(
                ...
                "VALUES (?, ?, ?, ?)",
                (device_id, now_ms, transcript, reply),
            )
    ...
    def revoke(self, device_id: str) -> None:
        ...
        try:
            ...
            changed = self.db.execute(
                "UPDATE memory_devices SET state='revoked' WHERE device_id=?",
                (device_id,),
            ).rowcount
            if changed != 1:
