...
class GatewayIntegrationTest(unittest.IsolatedAsyncioTestCase):
    async def test_ota_timeout_and_offline_fail_closed(self):
        ...
        connection.session.receive(
            Frame(MessageType.HELLO, 0, 7, 1, 0, 1, 0, struct.pack("!I", CAP_OTA))
        )
        with self.assertRaisesRegex(ProtocolError, "ota_timeout"):
            await connection.request_ota("a" * 64)
        self.assertEqual(
            Frame.decode(socket.sent[0]).message_type, MessageType.OTA_REQUEST
        )
        socket.open = False
    ...
    def setUpClass(cls) -> None:
        ...
        subprocess.run(
            [
                ...
                "-nodes",
                "-keyout",
                str(cls.untrusted_client_key),
                "-out",
                str(cls.untrusted_client_cert),
                "-days",
                "1",
                "-subj",
                "/CN=untrusted-client",
    ...
    async def asyncTearDown(self) -> None:
        ...
        if unexpected:
            kinds = [
                (
                    type(c.get("exception")).__name__,
                    getattr(c.get("exception"), "reason", None),
                )
                for c in unexpected
            ]
    ...
    async def handshake(
        self, websocket, client: ClientWire, capabilities=0
    ) -> tuple[Frame, Frame]:
        await websocket.send(
            client.frame(
                MessageType.HELLO,
                payload=struct.pack("!I", capabilities) if capabilities else b"",
            ).encode()
        )
    ...
    async def test_application_heartbeat_preserves_idle_and_active_turn(self) -> None:
        self.gateway.config = replace(
            self.gateway.config, heartbeat_interval_seconds=0.02
        )
        client = ClientWire()
    ...
    async def test_normal_turn_streams_fixed_pcm_and_ends_idle(self) -> None:
        ...
        async with self.connect() as websocket:
            ...
            self.assertFalse(
                any(e.get("event") == "playback_confirmed" for e in self.events)
            )
    ...
    async def _send_provider_turn(self, websocket, client, turn=1):
        ...
        await websocket.send(
            client.frame(MessageType.TURN_START, turn_id=turn).encode()
        )
        await websocket.send(
            client.frame(
                MessageType.AUDIO_UP, turn_id=turn, payload=b"\x01\x02" * 320
            ).encode()
        )
        await websocket.send(client.frame(MessageType.TURN_END, turn_id=turn).encode())
        self.assertEqual(
            Frame.decode(await websocket.recv()).message_type, MessageType.WINDOW_UPDATE
        )
    ...
    async def _mimo_endpoint(self, mode="normal"):
        ...
        async def handler(request):
            ...
            if model.endswith("-asr"):
                ...
                with wave.open(
                    io.BytesIO(base64.b64decode(encoded.split(",")[1])), "rb"
                ) as wav:
                    self.assertEqual(
                        wav.readframes(wav.getnframes()), b"\x01\x02" * 320
                    )
        ...
        provider = MiMoProvider(
            MiMoConfig("fixture-only", 16000, base_url=f"https://localhost:{port}/v1"),
    ...
    async def test_mimo_https_asr_chat_tts_through_real_wss(self):
        ...
        async with self.connect() as websocket:
            ...
            while not received or received[-1].message_type is not MessageType.TTS_END:
                received.append(
                    Frame.decode(await asyncio.wait_for(websocket.recv(), 3))
                )
            self.assertEqual(received[0].message_type, MessageType.TTS_START)
            audio = [
                frame
            ...
            self.assertEqual(
                b"".join(frame.payload for frame in audio), pcm + bytes(520)
            )
            self.assertEqual(
                [bool(frame.flags & Flag.END_OF_STREAM) for frame in audio],
                [False, False, True],
            )
    ...
    async def _drain_provider_turn(self, websocket):
        while True:
            ...
            if frame.message_type is MessageType.TTS_END:
                return
            self.assertIn(
                frame.message_type, (MessageType.TTS_START, MessageType.AUDIO_DOWN)
            )
    ...
    async def test_mimo_context_is_per_connection_and_playback_error_rolls_back(self):
        ...
        async with self.connect() as first, self.connect() as second:
            ...
            self.assertEqual(
                [r for r in requests if r["model"] == "mimo-v2.5"][-1]["messages"][1:],
                [{"role": "user", "content": "fixture-transcript"}],
            )
            await first.send(
                client.frame(
                    MessageType.ERROR, turn_id=2, payload=b"playback_failed"
                ).encode()
            )
            ...
            self.assertEqual(
                len([r for r in requests if r["model"] == "mimo-v2.5"][-1]["messages"]),
                4,
            )
    ...
    async def test_unconfirmed_playback_is_excluded_from_next_turn_context(self):
        ...
        async with self.connect() as peer:
            ...
            await self._drain_provider_turn(peer)
            self.assertEqual(
                [len(r["messages"]) for r in requests if r["model"] == "mimo-v2.5"],
                [2, 2],
            )
    ...
    async def test_cancelled_downlink_never_commits_generated_reply(self):
        ...
        async with self.connect() as websocket:
            ...
            await websocket.send(
                client.frame(
                    MessageType.AUDIO_UP, turn_id=1, payload=b"\x01\x02" * 320
                ).encode()
            )
            await websocket.send(client.frame(MessageType.TURN_END, turn_id=1).encode())
    ...
    async def test_provider_streams_before_eof_and_respects_credit(self):
        ...
        self.gateway.config = replace(
            self.gateway.config, downlink_window_timeout_ms=5000
        )
        ...
        async with self.connect() as websocket:
            ...
            await websocket.send(
                client.frame(
                    MessageType.AUDIO_UP, turn_id=1, payload=bytes(AUDIO_FRAME_BYTES)
                ).encode()
            )
            await websocket.send(client.frame(MessageType.TURN_END, turn_id=1).encode())
            self.assertEqual(
                Frame.decode(await websocket.recv()).message_type,
                MessageType.WINDOW_UPDATE,
            )
            self.assertEqual(
                Frame.decode(await websocket.recv()).message_type, MessageType.TTS_START
            ...
            await websocket.send(
                client.frame(
                    MessageType.WINDOW_UPDATE,
                    turn_id=1,
                    payload=encode_window_credit(2 * AUDIO_FRAME_BYTES),
                ).encode()
            )
            self.assertEqual(
                Frame.decode(await websocket.recv()).payload, b"\x02\x00" * 320
            )
            ...
            self.assertEqual(
                Frame.decode(await websocket.recv()).message_type, MessageType.TTS_END
            )
        await asyncio.wait_for(closed.wait(), 1)

    async def test_downlink_window_timeout_is_a_protocol_error(self):
        self.gateway.config = replace(
            self.gateway.config, downlink_window_timeout_ms=20
        )
        async with self.connect() as websocket:
            client = ClientWire()
            await self.handshake(websocket, client)
    ...
    async def test_uplink_limit_rejects_before_provider_invocation(self):
        ...
        self.gateway.config = replace(
            self.gateway.config, max_uplink_bytes=AUDIO_FRAME_BYTES
        )
        self.gateway.reply_factory = lambda: FixtureReplySession(provider)
        async with self.connect() as websocket:
            client = ClientWire()
            ...
            self.assertEqual(
                Frame.decode(await websocket.recv()).message_type, MessageType.ERROR
            )
            with self.assertRaises(ConnectionClosedError):
                await websocket.recv()
    ...
    async def test_registered_device_connection_lifecycle(self) -> None:
        ...
        context = self.mtls_client_context(
            self.trusted_client_cert, self.trusted_client_key
        )
        async with websockets.connect(
            uri, ssl=context, subprotocols=[SUBPROTOCOL]
        ) as first:
    ...
    async def console_fixture(
        self, capabilities=0, firmware_releases=None, memory_store=None, ota_store=False
    ):
        der = ssl.PEM_cert_to_DER_cert(self.trusted_client_cert.read_text())
        self.gateway.device_bindings = DeviceBindings(
            {hashlib.sha256(der).hexdigest(): "board-1", "a" * 64: "board-2"}
        )
        self.gateway.reply_factory = MiMoProvider(
            MiMoConfig(api_key="test-only-not-sent", tts_sample_rate=16000)
        ).new_conversation
        ...
        grants = tuple(
            ConsoleGrant(
                hashlib.sha256((char * 43).encode()).hexdigest(),
                "board-1",
                write,
                expiry,
            )
            for char, write, expiry in [
                ("w", True, now + 60000),
                ("r", False, now + 60000),
                ("e", True, now - 1),
            ]
    ...
    def console_mutation(self, snapshot, request_id="persona-1", **changes):
        ...
        value = dict(
            ...
            operation="persona_mode.set",
            arguments={"persona_mode": "quiet"},
        )
    ...
    async def test_console_volume_waits_for_board_and_caches_receipt(self):
        ...
        async with ClientSession(
            ...
            fetch = asyncio.create_task(
                client.get(url + "/snapshot", ssl=self.client_tls)
            )
            request = Frame.decode(await peer.recv())
            self.assertEqual(request.message_type, MessageType.VOLUME_GET)
            await peer.send(
                wire.frame(
                    MessageType.VOLUME_REPORT,
                    payload=struct.pack("!IiI", request.sequence, 0, 50),
                ).encode()
            )
            ...
            await peer.send(
                wire.frame(
                    MessageType.VOLUME_REPORT,
                    payload=struct.pack("!IiI", request.sequence, 0, 67),
                ).encode()
            )
            ...
            async with client.post(
                url + "/mutations", json=mutation, ssl=self.client_tls
            ) as response:
    ...
    async def test_console_volume_timeout_is_cached_and_query_recovers(self):
        ...
        async with ClientSession(
            ...
            await peer.send(
                wire.frame(
                    MessageType.VOLUME_REPORT,
                    payload=struct.pack("!IiI", lost.sequence, 0, 67),
                ).encode()
            )
            fetch = asyncio.create_task(
                client.get(url + "/snapshot", ssl=self.client_tls)
            ...
            await peer.send(
                wire.frame(
                    MessageType.VOLUME_REPORT,
                    payload=struct.pack("!IiI", query.sequence, 0, 67),
                ).encode()
            )
            updated = await (await fetch).json()
            self.assertEqual(updated["payload"]["volume_percent"], 67)
            self.assertGreater(
                updated["payload"]["revision"], snapshot["payload"]["revision"]
            )
    ...
    async def test_volume_request_matches_report_and_rejects_late_reply(self):
        ...
        await peer.send(
            wire.frame(
                MessageType.VOLUME_REPORT,
                payload=struct.pack("!IiI", request.sequence + 1, 0, 99),
            ).encode()
        )
        await peer.send(
            wire.frame(
        ...
        await peer.send(
            wire.frame(
                MessageType.VOLUME_REPORT,
                payload=struct.pack("!IiI", request.sequence, -16, 0xFFFFFFFF),
            ).encode()
        )
    ...
    async def test_remote_cancel_discards_crossing_pcm_and_stops_provider(self):
        ...
        self.assertEqual(
            Frame.decode(await peer.recv()).message_type, MessageType.WINDOW_UPDATE
        )
    ...
    async def test_console_https_persona_events_and_idempotency(self):
        ...
        async with ClientSession(headers=headers) as client:
            ...
            async with client.ws_connect(events_url, ssl=self.client_tls) as events:
                ...
                expected_mutation = json.loads(
                    (fixtures / "persona-mutation.json").read_text()
                )
                self.assertEqual(
                    dict(mutation, issued_at_ms=1000, expires_at_ms=31000),
                    expected_mutation,
                )
                async with client.post(
                    url + "/mutations",
                ...
                self.assertEqual(
                    self.gateway.device_connection(
                        "board-1"
                    ).reply_session.persona_mode,
                    "quiet",
                )
                ...
                async with client.post(
                    url + "/mutations", json=changed, ssl=self.client_tls
                ) as response:
                    self.assertEqual(response.status, 409)
                stale = dict(mutation, request_id="stale")
                async with client.post(
                    url + "/mutations", json=stale, ssl=self.client_tls
                ) as response:
                    self.assertEqual(
                        (await response.json())["error"], "revision_conflict"
                    )
                unsupported = self.console_mutation(
                    event,
                    "unsupported",
                    operation="volume.set",
                ...
                async with client.post(
                    url + "/mutations", json=unsupported, ssl=self.client_tls
                ) as response:
                    self.assertEqual(
                        (await response.json())["error"], "volume_not_supported"
                    )
            ...
            async with websockets.connect(
                ...
                async with client.get(
                    url + "/snapshot", ssl=self.client_tls
                ) as response:
                ...
                async with client.post(
                    ...
                    self.assertEqual(
                        (await response.json())["error"], "stale_generation"
                    )
    ...
    async def test_console_projects_authenticated_device_status(self):
        ...
        await peer.send(wire.frame(MessageType.STATUS_REPORT, payload=payload).encode())
        headers = {"Authorization": "Bearer " + "w" * 43}
        async with ClientSession(headers=headers) as client:
            for _ in range(100):
                async with client.get(
    ...
    async def test_console_projects_only_source_compatible_verified_releases(self):
        ...
        connection.device_status = StatusReport(
            ...
            firmware_version="18.6.389+449",
            firmware_root_sha256="b" * 64,
        )
        headers = {"Authorization": "Bearer " + "r" * 43}
        async with ClientSession(headers=headers) as client:
            snapshot = await (
                await client.get(url + "/snapshot", ssl=self.client_tls)
            ).json()
            generation = snapshot["generation"]
            async with client.get(
                f"{url}/firmware/releases?generation={generation}", ssl=self.client_tls
            ) as response:
                self.assertEqual(response.status, 200)
                self.assertEqual(response.headers["Cache-Control"], "no-store")
            ...
            for query, expected in (
                ...
                async with client.get(
                    f"{url}/firmware/releases?{query}", ssl=self.client_tls
                ) as response:
    ...
    async def test_console_ota_accepts_only_matching_report_and_projects_status(self):
        ...
        connection = self.gateway.device_connection("board-1")
        connection.device_status = StatusReport(
            None, None, None, None, "18.6.389+449", "b" * 64
        )
        headers = {"Authorization": "Bearer " + "w" * 43}
        async with ClientSession(headers=headers) as client:
            snapshot = await (
            ...
            request = Frame.decode(await peer.recv())
            self.assertEqual(
                (request.message_type, request.flags, request.turn_id, request.payload),
                (MessageType.OTA_REQUEST, 0, 0, b"\xaa" * 32),
            )
            self.assertFalse(submit.done())
            ...
            await peer.send(
                wire.frame(
                    ...
                    payload=struct.pack(
                        "!IiBBH32s", request.sequence + 1, 0, 1, 0, 0, b"\xaa" * 32
                    ),
                ).encode()
            )
            await peer.send(
                wire.frame(
                    MessageType.OTA_REPORT,
                    payload=struct.pack(
                        "!IiBBH32s", request.sequence, 0, 1, 0, 0, b"\xbb" * 32
                    ),
                ).encode()
            )
            ...
            await peer.send(
                wire.frame(
                    MessageType.OTA_REPORT,
                    payload=struct.pack(
                        "!IiBBH32s", request.sequence, 0, 1, 3, 0, b"\xaa" * 32
                    ),
                ).encode()
            )
            receipt = await (await submit).json()
            ...
            await peer.send(
                wire.frame(
                    MessageType.OTA_REPORT,
                    payload=struct.pack(
                        "!IiBBH32s", request.sequence, 0, 1, 2, 0, b"\xaa" * 32
                    ),
                ).encode()
            )
            await peer.send(
                wire.frame(
                    MessageType.OTA_REPORT,
                    payload=struct.pack(
                        "!IiBBH32s", request.sequence, 0, 2, 4, 0, b"\xaa" * 32
                    ),
                ).encode()
            )
            await peer.send(
                wire.frame(
                    MessageType.OTA_REPORT,
                    payload=struct.pack(
                        "!IiBBH32s", request.sequence, 0, 1, 99, 0, b"\xaa" * 32
                    ),
                ).encode()
            )
            for _ in range(20):
                if (
                    connection.ota_status is not None
                    and connection.ota_status.phase == 2
                ):
                    break
                await asyncio.sleep(0.01)
            self.assertEqual(
                (connection.ota_status.phase, connection.ota_status.progress_percent),
                (2, 4),
            )
            await peer.send(
                wire.frame(
                    MessageType.OTA_REPORT,
                    payload=struct.pack(
                        "!IiBBH32s", request.sequence, 0, 6, 100, 0, b"\xaa" * 32
                    ),
                ).encode()
            )
            for _ in range(20):
            ...
            await peer.send(
                wire.frame(
                    MessageType.OTA_REPORT,
                    payload=struct.pack(
                        "!IiBBH32s", request.sequence, 0, 5, 90, 0, b"\xaa" * 32
                    ),
                ).encode()
            )
            await asyncio.sleep(0)
            ...
            blocked = self.console_mutation(
                ...
                "ota-busy-after-terminal",
                operation="firmware.update",
                arguments={"release_manifest_sha256": "a" * 64},
            )
            response = await client.post(
                url + "/mutations", json=blocked, ssl=self.client_tls
            )
            blocked_receipt = await response.json()
            self.assertEqual(
                (blocked_receipt["error"], blocked_receipt["revision"]),
                ("revision_conflict", 1),
            )
        ...
        async with ClientSession(headers=headers) as client:
            ...
            await peer.send(
                wire.frame(
                    MessageType.OTA_REPORT,
                    payload=struct.pack(
                        "!IiBBH32s", request.sequence, -9, 8, 17, 0, b"\xaa" * 32
                    ),
                ).encode()
            )
            receipt = await (await submit).json()
            self.assertEqual(
                (receipt["status"], receipt["error"], receipt["revision"]),
        ...
        async with ClientSession(headers=headers) as client:
            ...
            await peer.send(
                wire.frame(
                    MessageType.OTA_REPORT,
                    payload=struct.pack(
                        "!IiBBH32s", request.sequence, 0, 1, 9, 0, b"\xaa" * 32
                    ),
                ).encode()
            )
            for _ in range(20):
                ...
                await asyncio.sleep(0.01)
            self.assertEqual(
                projected["payload"]["update"]["target_version"], "18.6.390+450"
            )
        ...
        async with ClientSession(headers=headers) as client:
            ...
            await peer.send(
                wire.frame(
                    MessageType.OTA_REPORT,
                    payload=struct.pack(
                        "!IiBBH32s", request.sequence, 0, 1, 19, 0, b"\xaa" * 32
                    ),
                ).encode()
            )
            self.assertEqual((await (await submit).json())["status"], "accepted")
            self.assertEqual(
                service.ota_transactions.load("board-1").state, "downloading"
            )
            ...
            for _ in range(100):
                ...
                await asyncio.sleep(0.005)
            self.assertEqual(
                service.ota_transactions.load("board-1").state, "uncertain"
            )
            ...
            async with websockets.connect(
                ...
                await reconnected.send(
                    reconnect_wire.frame(
                        MessageType.STATUS_REPORT, payload=status_payload
                    ).encode()
                )
                active = None
                for _ in range(20):
                ...
                self.assertEqual(
                    (resumed.message_type, resumed.payload),
    ...
    async def test_console_ota_terminal_report_survives_disconnect_until_status(self):
        ...
        async with ClientSession(headers=headers) as client:
            ...
            await self.wait_for_summary()
            self.assertEqual(
                service.ota_transactions.load("board-1").state, "confirming"
            )
    ...
    async def test_console_ota_rejects_registry_capability_busy_and_timeout(self):
        ...
        async with ClientSession(headers=headers) as client:
            ...
            response = await client.post(
                url + "/mutations", json=invalid, ssl=self.client_tls
            )
            self.assertEqual(
                (await response.json())["error"], "update_manifest_invalid"
            )
            invalid_digest = self.console_mutation(
                snapshot,
                "ota-sentinel",
                operation="firmware.update",
            ...
            response = await client.post(
                url + "/mutations", json=invalid_digest, ssl=self.client_tls
            )
            ...
            response = await client.post(
                url + "/mutations", json=no_cap, ssl=self.client_tls
            )
            ...
            response = await client.post(
                url + "/mutations", json=busy, ssl=self.client_tls
            )
            self.assertEqual(response.status, 409)
        with self.assertRaises(asyncio.TimeoutError):
            await asyncio.wait_for(peer.recv(), 0.03)
    ...
    async def test_console_memory_clear_and_revoke_are_device_scoped(self):
        ...
        async with ClientSession(headers=headers) as client:
            ...
            self.assertEqual(
                snapshot["payload"]["permissions"]["long_term_memory"], "allowed"
            )
            ...
            async with client.post(
                url + "/mutations", json=clear, ssl=self.client_tls
            ) as response:
                receipt = await response.json()
            self.assertEqual(receipt["status"], "accepted")
            ...
            self.assertEqual(
                current["payload"]["permissions"]["long_term_memory"], "allowed"
            )
            ...
            async with client.post(
                url + "/mutations", json=enable, ssl=self.client_tls
            ) as response:
            ...
            async with client.post(
                url + "/mutations", json=revoke, ssl=self.client_tls
            ) as response:
            ...
            self.assertEqual(
                denied["payload"]["permissions"]["long_term_memory"], "denied"
            )
    ...
    async def test_console_cancel_revision_binding_and_concurrent_retry(self):
        ...
        async with ClientSession(headers=headers) as client:
            ...
            old = self.console_mutation(
                idle, "old", operation="turn.cancel", arguments={}
            )
            async with client.post(
                url + "/mutations", json=old, ssl=self.client_tls
            ) as response:
                self.assertEqual((await response.json())["error"], "revision_conflict")
            async with client.get(url + "/snapshot", ssl=self.client_tls) as response:
                active = await response.json()
            mutation = self.console_mutation(
                active, "cancel-1", operation="turn.cancel", arguments={}
            )
            ...
            cancel = Frame.decode(await peer.recv())
            self.assertEqual(
                (cancel.message_type, cancel.turn_id), (MessageType.CANCEL, 1)
            )
            self.assertEqual(
                self.gateway.device_connection("board-1").metrics.turns_cancelled, 1
            )
            async with client.get(url + "/snapshot", ssl=self.client_tls) as response:
            ...
            self.assertEqual(
                sum(e.get("event") == "remote_cancel_confirmed" for e in self.events), 1
            )
            await peer.send(wire.frame(MessageType.TURN_START, turn_id=2).encode())
            await peer.send(
                wire.frame(
                    MessageType.AUDIO_UP, turn_id=2, payload=bytes(AUDIO_FRAME_BYTES)
                ).encode()
            )
            await peer.recv()
            # No snapshot poll in this new turn: the endpoint must still notice it.
            stale = self.console_mutation(
                after_cancel, "late", operation="turn.cancel", arguments={}
            )
            async with client.post(
                url + "/mutations", json=stale, ssl=self.client_tls
            ) as response:
                self.assertEqual((await response.json())["error"], "revision_conflict")
            self.assertEqual(
                self.gateway.device_connection("board-1").session.state,
                SessionState.UPLINK,
            )
            self.assertEqual(
                self.gateway.device_connection("board-1").metrics.turns_cancelled, 1
    ...
    async def test_console_playback_waits_for_ack_and_reports_unknown_on_timeout(self):
        ...
        async with ClientSession(
            ...
            async with client.post(
                ...
                self.assertEqual(response.status, 409)
            connection.config = replace(
                connection.config, playback_ack_timeout_seconds=0.02
            )
            await asyncio.sleep(0.03)
            async with client.get(url + "/snapshot", ssl=self.client_tls) as response:
                self.assertEqual(
                    (await response.json())["payload"]["turn"], "playback_unconfirmed"
                )
            ...
            cancel = self.console_mutation(
                snapshot, "idle-cancel", operation="turn.cancel", arguments={}
            )
            async with client.post(
                url + "/mutations", json=cancel, ssl=self.client_tls
            ) as response:
                self.assertEqual(response.status, 409)
            self.assertEqual(len(connection.reply_session._history), 1)
    ...
    async def test_cancel_ack_timeout_is_unknown_and_late_ack_recovers(self):
        ...
        await connection.cancel_turn(1)
        self.assertEqual(
            Frame.decode(await peer.recv()).message_type, MessageType.CANCEL
        )
        await asyncio.sleep(0.03)
        async with ClientSession(
            headers={"Authorization": "Bearer " + "w" * 43}
            ...
            async with client.get(url + "/snapshot", ssl=self.client_tls) as response:
                self.assertEqual(
                    (await response.json())["payload"]["turn"], "cancel_unconfirmed"
                )
            self.assertEqual(connection.cancel_pending_turn, 1)
            self.assertFalse(
                any(e.get("event") == "remote_cancel_confirmed" for e in self.events)
            )
            await peer.send(wire.frame(MessageType.ACK, turn_id=1).encode())
            await peer.send(
                wire.frame(
    ...
    async def test_console_authorization_expiry_schema_and_busy_turn(self):
        ...
        async with ClientSession() as client:
            for char in ("", "x", "e"):
                ...
                async with client.get(
                    url + "/snapshot", headers=headers, ssl=self.client_tls
                ) as response:
                    self.assertEqual(response.status, 401)
            headers = {"Authorization": "Bearer " + "w" * 43}
            async with client.get(
                url.replace("board-1", "board-2") + "/snapshot",
                headers=headers,
                ssl=self.client_tls,
            ) as response:
            ...
            for change in (
                ...
                async with client.post(
                    url + "/mutations",
                    json=dict(mutation, **change),
                    headers=headers,
                    ssl=self.client_tls,
                ) as response:
                    self.assertEqual(response.status, 400)
            expired = dict(
                mutation, request_id="expired", issued_at_ms=1, expires_at_ms=2
            )
            async with client.post(
                url + "/mutations", json=expired, headers=headers, ssl=self.client_tls
            ) as response:
            ...
            async with client.post(
                url + "/mutations", json=mutation, headers=headers, ssl=self.client_tls
            ) as response:
                self.assertEqual(response.status, 409)
    ...
    def _firmware_content_fixture(self, *, compressed=False):
        ...
        base = {
            ...
            "security_counter": 450,
            "cp": {
                "uri": "images/cp/cp.bin",
                "size": len(cp),
                "sha256": hashlib.sha256(cp).hexdigest(),
            },
            "ap": {
                "uri": "images/ap/ap.bin",
                "size": len(ap),
        ...
        with zipfile.ZipFile(
            package, "w", compression=compression, allowZip64=False
        ) as archive:
    ...
    async def test_firmware_content_requires_live_bound_ota_and_serves_bounded_ranges(
        self,
    ) -> None:
    ...
    def test_bind_scope_gate_table(self) -> None:
        ...
        for host, client_ca in (
            ("192.168.1.7", None),
            ("0.0.0.0", self.ca_path),
            ("::", self.ca_path),
            ("255.255.255.255", self.ca_path),
            ("224.0.0.1", self.ca_path),
            ("gateway.example", self.ca_path),
        ):
