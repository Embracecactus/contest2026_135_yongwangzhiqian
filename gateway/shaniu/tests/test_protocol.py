...
class FrameTest(unittest.TestCase):
    def test_ota_capability_and_report_contract(self) -> None:
        ...
        for result, phase, progress, reserved in (
            (1, 1, 0, 0),
            (-1, 1, 0, 0),
            (0, 8, 0, 0),
            (0, 1, 101, 0),
    ...
    def test_status_report_requires_declared_capability_and_valid_contract(
        self,
    ) -> None:
        ...
        self.assertEqual(
            Frame.decode(
                Frame(MessageType.STATUS_REPORT, 0, 7, 1, 0, 2, 0, payload_v2).encode()
            ).payload,
            payload_v2,
        )
        ...
        for changed in (
            ...
            struct.pack("!BBBBIHHHHI32s", 2, 73, 1, 4, 3800, 1, 2, 3, 0, 4, bytes(32)),
            struct.pack(
                "!BBBBIHHHHI32s",
                2,
                73,
            ...
            with self.subTest(payload=changed), self.assertRaisesRegex(
                ProtocolError, "invalid_status_report"
            ):
        ...
        with self.assertRaisesRegex(ProtocolError, "invalid_status_report"):
            Frame(
                MessageType.STATUS_REPORT, int(Flag.SYNTHETIC), 7, 1, 0, 2, 0, payload
            ).encode()
    ...
    def test_volume_capabilities_and_report_contract(self) -> None:
        ...
        session = GatewaySession()
        session.receive(
            Frame(MessageType.HELLO, 0, 7, 1, 0, 1, 0, struct.pack("!I", CAP_VOLUME))
        )
        ...
        self.assertEqual(
            session.make_volume_request().message_type, MessageType.VOLUME_GET
        )
    ...
    def test_volume_rejects_invalid_wire_values(self) -> None:
            ...
            with self.subTest(values=(request, result, volume)):
                with self.assertRaisesRegex(ProtocolError, "invalid_volume_report"):
                    Frame(
                        ...
                        2,
                        0,
                        struct.pack("!IiI", request, result, volume),
                    ).encode()
        failure = Frame(
            MessageType.VOLUME_REPORT,
            0,
        ...
        for kind, turn, payload in (
            (MessageType.VOLUME_GET, 1, b""),
...
class GatewaySessionTest(unittest.TestCase):
    ...
    def test_remote_cancel_keeps_sequence_and_new_turn_boundaries(self) -> None:
        ...
        crossed = Frame(
            MessageType.AUDIO_UP, 0, 7, 3, 1, 3, 1, bytes(AUDIO_FRAME_BYTES)
        )
        self.assertEqual(
            self.session.receive(crossed).event, SessionEvent.DISCARDED_AUDIO
        )
        with self.assertRaisesRegex(ProtocolError, "sequence_replay"):
        ...
        with self.assertRaisesRegex(ProtocolError, "stale_turn"):
            self.session.receive(
                Frame(MessageType.AUDIO_UP, 0, 7, 3, 1, 6, 1, bytes(AUDIO_FRAME_BYTES))
            )
