...
class MemoryStoreTest(unittest.TestCase):
    def make_store(self, directory, *, devices=("board-1",), clock=None, **kwargs):
        return MemoryStore(
            Path(directory) / "memory.db", devices, clock_ms=clock or Clock(), **kwargs
        )
    ...
    def test_expiry_record_limit_and_context_budget(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            store = self.make_store(
                directory, clock=clock, retention_ms=100, max_records_per_device=2
            )
            ...
            self.assertEqual(
                store.context("board-1"), (("middle", "reply"), ("newest", "reply"))
            )
            store.close()
