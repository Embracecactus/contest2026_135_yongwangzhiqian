...
class ConsoleConfigurationTest(unittest.TestCase):
    def test_grant_file_validation(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for invalid in (
                [grant, grant],
                [dict(grant, write=1)],
                [dict(grant, expires_at_ms=True)],
                [dict(grant, extra=True)],
                [],
            ):
    ...
    def test_persona_persists_and_corrupt_state_fails_closed(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            state.db.execute(
                "UPDATE console_persona SET mode='invalid' WHERE device_id='board-1'"
            )
            state.db.commit()
    ...
    def test_firmware_registry_is_strict_and_source_identity_bound(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            save(dict(format="shaniu.firmware-release-registry/1", releases=[release]))
            ...
            self.assertEqual(
                registry.compatible("board-1", "18.6.389+449", root),
                (FirmwareRelease(**release),),
            )
            self.assertEqual(registry.compatible("board-1", None, root), ())
            self.assertEqual(registry.compatible("board-1", "18.6.388+448", root), ())
            self.assertEqual(
                registry.compatible("board-1", "18.6.389+449", "0" * 64), ()
            )
            self.assertEqual(registry.compatible("board-2", "18.6.389+449", root), ())
            ...
            invalid = (
                ...
                dict(
                    format="shaniu.firmware-release-registry/1",
